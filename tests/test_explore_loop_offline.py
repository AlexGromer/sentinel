...
class ClickEx:
    ...
    def call(self, m, **p):
        ...
        if m == "browser.probe":
            return {"count": 1}
        if m == "browser.routes":
            # ADR-135: второй источник фронтира. Эта цель маршрутов не меняет — журнал пуст, и
            # пустота ЗДЕСЬ значима: она держит замер этого файла на прежнем канале.
            return {"routes": [], "dropped": 0, "journal": True}
        if m.startswith("browser."):
            ...
            # пустоту, и гейт оставался ЗЕЛЁНЫМ над механизмом, которого в замере нет вовсе.
            # Замерено 2026-08-24: проводка второго источника фронтира была внесена целиком, и все
            # четыре гейта обхода прошли, не заметив.
            #
            # Отсечка именно по `browser.` — граница реплики: она моделирует поверхность БРАУЗЕРА.
            # `initialize` и `shutdown` — протокол транспорта, а не способность, и они здесь так же
            # безразличны, как в перечне не-верб гейта контракта.
