...
def main() -> int:
    ...
    # решением (`POST /v1/login` в apiRoutesWithoutCLI). Глагол, способный предъявить лишь машинный
    # токен, краснел бы 403 при каждом вызове — это видимость пути, а не путь. Причина записана в
    # `missing.cli` самой записи, поэтому здесь она сверяема, а не обещана.
    NOT_THREE_WAY_TODAY = 27   # ⚠ и это тоже равенство — иначе доля падает молча
