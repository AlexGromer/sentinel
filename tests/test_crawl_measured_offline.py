...
LIVE = {
    ...
    # chain — ПЕРЕЗАМЕРЕН ТРЕТИЙ РАЗ, и каждый раз по делу. Было 5 шагов и «нет кандидатов» на одной
    # странице; стало 40 шагов, 11 страниц и упор в потолок; теперь — 18 шагов и ЧЕСТНАЯ сходимость
    # при тех же 11 страницах.
    #
    # ⚠ ПРИЧИНА ПАДЕНИЯ ЧИСЛА ШАГОВ — ADR-140, И ЭТО НЕ ПОТЕРЯ. Из прежних 40 шагов **22 были
    # навигациями на ОДИН адрес**: голая дверь уводила роутером на свой маршрут по умолчанию,
    # `ground` снимал с фронтира адрес ПРИЗЕМЛЕНИЯ, а запрошенный оставался — и обход ходил туда,
    # пока не кончался бюджет. Теперь навигация ОДНА. Досягаемость проверена отдельно: при цели
    # покрытия 1.0 та же дверь даёт 17 проработанных и coverage 0.9444 — ровно прежний результат —
    # за 19 шагов вместо 40. При цели по умолчанию 0.85 обход честно останавливается, ДОСТИГНУВ ЕЁ,
    # и `exercised` 16 против 17 — это не недоработка, а отсутствие работы сверх заказанной.
    "chain.html": {"steps": 18, "coverage": 0.8889, "seen": 18, "exercised": 16, "navigations": 1,
                   "pages": 11, "reason": "converged",
                   "plan_hash_portable": "c3c27d970f0faef6da002a938bc79d89ccf97c876c46edee05a0f66a3cb54fc2"},
}
...
# ⚠ ПЕРЕЗАМЕРЕНО ADR-140, И ЭТО ГЛАВНОЕ ЧИСЛО ВОЛНЫ. Было: 200 шагов, упор в бюджет. Стало: **58
# шагов и `no_candidates`** при ТЕХ ЖЕ 23 страницах и ТОМ ЖЕ покрытии 0.6667. Из прежних 200 шагов
# **144 были навигациями на ОДИН адрес** — голый `index.html`, с которого роутер уводит на `#/home`:
# `ground` снимал с фронтира адрес приземления, запрошенный оставался, а якорь `href="index.html"`
# подавал его заново на КАЖДОМ восприятии. 143 шага из 200 — 72 % бюджета — уходили в холостой ход
# без единого клика между ними, и `reason` при этом рапортовал `max_steps`, то есть «не хватило
# потолка», хотя обход стоял на месте.
#
# ⚠ СЛЕДСТВИЕ ДЛЯ СМЫСЛА ЭТОГО ФАЙЛА: на поднятом бюджете цель БОЛЬШЕ НЕ УПИРАЕТСЯ в бюджет — она
# кончается раньше него. Утверждение «M3 про бюджет» на этой двери перестало быть верным, и тесты
# ниже переписаны под то, что есть, а не под то, что было.
LIVE_INDEX_AT_200 = {"steps": 58, "coverage": 0.6667, "reason": "no_candidates", "pages": 23,
                     "plan_hash_portable": "6587a5e8ca8fb437be77be1ed07d7263d1459991254574d0d2f910a0df6fbf15"}
...
def _walk(door: str, max_steps=None) -> dict:
    ...
    try:
        ...
        out = {
            ...
            # ADR-140: сами АДРЕСА, а не только их число. Счётчик не отличает «сходил на десять
            # разных мест» от «десять раз сходил в одно», а именно эта разница и была дефектом.
            "nav_urls": list(ex.navigations),
...
def test_the_frontier_takes_route_anchors_now_and_still_cannot_see_a_pushstate_route():
    ...
    # ⚠ КОНТРОЛЬ ПЕРЕВЁРНУТ ADR-140, И УТВЕРЖДЕНИЕ СТАЛО СИЛЬНЕЕ, А НЕ СЛАБЕЕ.
    #
    # Прежняя строка требовала `reason is None and steps == 200` — то есть чтобы обход УПИРАЛСЯ в
    # бюджет. Она стояла тут затем, чтобы доля выше говорила о фронтире, а не о нехватке шагов, — но
    # ровно поэтому и не могла ничего доказать: пока бюджет кончается, «карта мала» имеет ДВЕ
    # возможные причины, и различить их было нечем.
    #
    # Замерено (ADR-140): 143 из 200 шагов уходили в холостую навигацию на один адрес. Теперь обход
    # заканчивает САМ на 58-м шаге с `no_candidates` — бюджета хватает с запасом, — а карта всё равно
    # держит 23 из 59 адресуемых. Значит меньшинство объясняется ИМЕННО слепотой фронтира, и это
    # доказано, а не предположено.
    assert lifted["reason"] == "no_candidates" and lifted["steps"] < 200, (
        f"на бюджете 200 обход снова израсходовал {lifted['steps']} шагов и кончился по причине "
        f"{lifted['reason']!r} — вернулся холостой ход, и доля выше опять говорит о нехватке "
        f"бюджета, а не о фронтире")
...
def test_the_checkout_chain_walks_both_branches_and_now_finishes_instead_of_dying_of_the_budget():
    ...
    ⚠ ПЕРЕПИСАН ВТОРИЧНО (ADR-140), И ПРЕЖНЯЯ ЦЕНА ОТМЕНЕНА. Абзац выше говорил: «обход теперь
    умирает от ПОТОЛКА, а не от идти-некуда, и это ХУДШЕЕ окончание». Замер показал, что умирал он
    не от потолка, а от собственной ошибки: **22 из 40 шагов были навигациями на ОДИН адрес** —
    голая дверь уводит роутером на свой маршрут, `ground` снимал с фронтира адрес ПРИЗЕМЛЕНИЯ, а
    запрошенный оставался навсегда. Теперь навигация ОДНА, обход проходит те же 11 страниц за
    **18 шагов** и заканчивается ЧЕСТНОЙ сходимостью.
    ⚠ Досягаемость при этом НЕ ПОТЕРЯНА, и это замерено отдельно: при цели покрытия 1.0 та же дверь
    даёт 17 проработанных и coverage 0.9444 — прежний результат — за 19 шагов. При цели 0.85 обход
    останавливается, ДОСТИГНУВ ЕЁ; `exercised` 16 вместо 17 — не недоработка, а отсутствие работы
    сверх заказанной."""
    r = _walk("chain.html")
    ...
    # ⚠ УТВЕРЖДЕНИЕ ПЕРЕВЁРНУТО (ADR-140): раньше здесь требовался УПОР В ПОТОЛОК, теперь —
    # окончание РАНЬШЕ него. Это не ослабление: потолок был симптомом холостого хода, а не свойством
    # цели, и требовать его значило бы охранять дефект.
    assert r["steps"] < DEFAULT_MAX_STEPS, (
        f"обход снова израсходовал весь потолок ({r['steps']} из {DEFAULT_MAX_STEPS}) — вернулся "
        f"холостой ход по фронтиру, который ADR-140 убрал")
    # ⚠ И ПОЛ, без которого предыдущая строка бессодержательна: «мало шагов» само по себе может
    # означать и схлопнувшийся обход. Дверь обязана быть пройдена, а не брошена.
    assert r["pages"] >= 11 and r["seen"] >= 18, (
        f"дверь пройдена хуже, чем прежде: страниц {r['pages']}, увидено {r['seen']} — это уже не "
        f"экономия бюджета, а потеря досягаемости")
    # Причина окончания, названная своим словом В АРТЕФАКТЕ.
    c = r["completeness"]
    assert c.get("reason") == "converged", (
        f"обход закончился не сходимостью: {c} — если это снова `max_steps`, вернулся холостой ход")
...
def test_the_walk_never_asks_for_the_same_address_twice():
    """ADR-140. Инвариант, а не число: адрес, на который обход СХОДИЛ, второй раз не запрашивается.

    ЧТО ЭТО ЛОВИТ, ЗАМЕРЕНО. Голая дверь (`index.html` без фрагмента) уводит роутером на свой
    маршрут по умолчанию. `ground` чистит фронтир адресом ПРИЗЕМЛЕНИЯ, поэтому запрошенный оставался
    в нём навсегда, а якорь `href="index.html"` подавал его заново на КАЖДОМ восприятии. Итог на
    бюджете 200: **144 навигации из 200 шагов на ОДИН адрес, 143 холостых шага — 72 % бюджета**, без
    единого клика между ними.

    ⚠ УТВЕРЖДЕНИЕ ПРО МНОЖЕСТВО, А НЕ ПРО СЧЁТЧИК. Прежний `navigations` считал вызовы и был к этому
    слеп: десять походов в одно место и десять в разные давали одно число. Здесь сравниваются
    адреса, поэтому мутация, вернувшая холостой ход, красит эту строку независимо от того, сколько
    шагов успел потратить прогон.
    """
    for door, budget in (("index.html", None), ("index.html", 200), ("chain.html", None),
                         ("cards.html", None)):
        r = _walk(door, max_steps=budget)
        urls = r["nav_urls"]
        dupes = {u: urls.count(u) for u in set(urls) if urls.count(u) > 1}
        assert not dupes, (
            f"{door} (бюджет {budget or 'по умолчанию'}): обход запрашивал один и тот же адрес по "
            f"нескольку раз — {dupes}. Это холостой ход по фронтиру, который ADR-140 убрал: адрес, "
            f"с которого приложение уводит себя само, снова не покидает фронтир")


def test_the_reason_a_walk_ended_is_the_true_one():
    """ADR-140. Причина окончания — то, ЗА ЧЕМ человек приходит в артефакт, и она врала.

    Обход, простоявший 143 шага на одном адресе, рапортовал `max_steps` — «не хватило потолка», —
    то есть предлагал читателю поднять бюджет там, где бюджет был ни при чём. Утверждение связывает
    ДВЕ наблюдаемые величины: если шагов израсходовано меньше потолка, причиной не может быть
    потолок; если израсходованы все — она обязана быть им.
    """
    for door, budget in (("index.html", None), ("index.html", 200), ("chain.html", None),
                         ("cards.html", None)):
        r = _walk(door, max_steps=budget)
        cap = budget or DEFAULT_MAX_STEPS
        reason = (r["completeness"] or {}).get("reason")
        if r["steps"] < cap:
            assert reason != "max_steps", (
                f"{door} (бюджет {cap}): обход израсходовал {r['steps']} шагов и всё же объявил "
                f"причиной потолок — читателю предлагается поднять бюджет, который не кончался")
        else:
            assert reason == "max_steps", (
                f"{door} (бюджет {cap}): обход израсходовал ВЕСЬ потолок, но назвал причиной "
                f"{reason!r} — тогда непонятно, сколько работы осталось за кадром")
