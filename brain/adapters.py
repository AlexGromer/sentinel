...
class StorageStateAuth(EnvBlockAdapter):
    ...
    #
    # ⚠ У `login_plan` ПОЯВИЛОСЬ СОБСТВЕННОЕ ИМЯ — И ТОЛЬКО ПОТОМУ, ЧТО ПОЯВИЛСЯ ПОТРЕБИТЕЛЬ (W17,
    # M9.1_CONTRACT §4.1). До этого имени не давали НАМЕРЕННО и это было записано здесь: ключ
    # принимался схемой, показывался формой и писался в `run.yaml`, а читать его было некому —
    # завести имя значило бы перенести мёртвую ручку на новый адрес и выдать это за починку.
    # Теперь `brain/__main__.py` исполняет названный план ПЕРЕД основным прогоном и отдаёт его сессию
    # основному прогону, поэтому доставка стала осмысленной.
    #
    # Имя НЕ `PLAN_FILE`: это имя ПЛАНА ПРОГОНА, который replay/baseline читает и исполняет, и
    # agentctl пишет его безусловно на каждом replay-пути. Столкновение имён закрыто ADR-172 и
    # обратно не заводится: план входа и план прогона — два разных предмета.
    _ENV = {"storage_state": "STORAGE_STATE", "storage_state_save": "STORAGE_STATE_SAVE",
            "login_plan": "LOGIN_PLAN", "pw_no_trace": "PW_NO_TRACE"}
