...
def build_graph(ex, planner, tx_write, scenario_head=None, rc=None, robots=None):
    ...
    def act(state: RunState) -> dict:
        ...
        if p["action_type"] == "click":
            ...
            #
            # Без этой строки адрес, с которого приложение уводит СЕБЯ САМО, не покидает фронтир
            # НИКОГДА, и обход крутится на нём до конца бюджета. Механика замерена целиком
            # (`testdata/site-spa`, потолок 200): во фронтире лежит голый `index.html`; роутер на
            # загрузке уходит на `#/home`; `ground` чистит фронтир адресом ПРИЗЕМЛЕНИЯ
            # (`frontier = [f for f in frontier if f != path]`), то есть снимает `#/home`, а голый
            # адрес оставляет; `visited_paths` копит тоже приземления, поэтому `admit_to_frontier`
            # пускает его заново — и якорь `href="index.html"` подаёт его на КАЖДОМ восприятии.
            # Итог: **144 навигации из 200 шагов на ОДИН адрес, 72 % бюджета**, ноль кликов между
            # ними, а `completeness.reason` при этом рапортует `max_steps` — «не хватило потолка»,
            # хотя обход стоял на месте.
            #
            # Это тот же класс, что ADR-134 и ADR-137: идентичность «что мы СДЕЛАЛИ» разошлась с
            # идентичностью «о чём мы ПРОСИЛИ». Лечение там же, где и там, — привести их к одной оси,
            # а не добавить исключение под конкретный случай. Записывается ПОПЫТКА, а не успех:
            # адрес, на который мы сходили и были уведены, спрашивать второй раз незачем — ответ
            # будет тот же, и это ровно то, что подтвердил замер 144 повторами.
            # ⚠ ОБА СПИСКА, И ЭТО НЕ ПЕРЕСТРАХОВКА — ПЕРВАЯ РЕДАКЦИЯ ПРАВКИ ТРОГАЛА ТОЛЬКО `visited`
            # И НЕ ИЗМЕНИЛА НИЧЕГО (замерено: те же 144 навигации). `visited` охраняет ДОПУСК новых
            # адресов, а этот адрес уже ЛЕЖИТ во фронтире — попасть туда заново ему и не требуется.
            # Поэтому снимается он ЗДЕСЬ, а `visited` держит его снаружи после того, как якорь
            # предложит его в следующий раз. Одно без другого не работает: без снятия он крутится,
            # без `visited` возвращается на первом же восприятии.
            _asked = page_identity(p.get("target") or "")
            if _asked:
                moved_to = {
                    "visited_paths": list(dict.fromkeys(
                        list(state.get("visited_paths", [])) + [_asked])),
                    "nav_frontier": [f for f in state.get("nav_frontier", []) if f != _asked],
                }
