...
async function dispatchInner(method: string, params: Record<string, unknown>): Promise<unknown> {
  switch (method) {
    ...
    case 'browser.navigate': {
      ...
      // ADR-145: `same_document` — навигация НЕ заменила документ (сменился только фрагмент, либо
      // адрес совпал с текущим И нёс фрагмент). До этой строки у исполнителя не было СЛОВА для
      // разницы между «goto перезагрузил приложение» и «goto не сделал ничего», и запись реестра
      // [EXECUTOR-GOTO-IS-UNCONDITIONAL] называла именно это: «её лечение должно отличать
      // возврат-как-работу от goto-на-том-же-месте — а этого различия у исполнителя сегодня нет».
      //
      // ⚠ ЭТО НЕ ДОГАДКА ПО `resp`, А ЗАМЕРЕННОЕ СОВПАДЕНИЕ ТРЁХ НЕЗАВИСИМЫХ НАБЛЮДЕНИЙ.
      // Замер 2026-09-01 на `testdata/fixtures/l13-routes.html`, пять случаев (идентичный адрес без
      // фрагмента · идентичный с фрагментом · фрагмент добавлен · фрагмент изменён · другой путь):
      // переживание метки в `window`, неизменность `performance.timeOrigin` и `resp === null`
      // ответили ОДИНАКОВО во всех пяти. Поэтому `resp === null` берётся как признак, а гейт
      // `routes.test.ts` сверяет его с независимым наблюдением, а не повторяет эту же формулу.
      //
      // ⚠ ПРОПУСК `goto` НЕ РЕАЛИЗОВАН, И ЭТО РЕШЕНИЕ, А НЕ НЕДОДЕЛКА. Лечение «не ходить, если уже
      // там» опровергнуто замером ADR-140: возврат на стартовый адрес ПЕРЕЗАПУСКАЕТ приложение и
      // открывает контролы, недостижимые из глубины маршрута (−1 страница, −4 клика при пропуске).
      // А перезамер всего корпуса фикстур 2026-09-01 дал НОЛЬ холостых goto: `site`, `site-v2`,
      // `site-v3` — по три навигации, ни одной на текущий адрес; `site-spa` — две, и повторная
      // ведёт на адрес БЕЗ фрагмента, то есть перезагружает, то есть работает. Пропускать нечего;
      // код без наблюдаемого случая нечем проверить. Здесь появляется ФАКТ, а решение, что с ним
      // делать, принимает вызывающий, когда случай появится.
      const sameDocument = resp === null;
      return {
        url: page!.url(),
        title: await page!.title(),
        // `status` остаётся как был (ADR-138 читает его на 404). Его `null` и `same_document: true`
        // — одно и то же событие, названное дважды: старое имя оставлено, чтобы не двигать
        // читателей, новое заведено, чтобы факт перестал быть побочным смыслом чужого поля.
