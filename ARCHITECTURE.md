...
| ADR-020 | 2026-06-25 | M7 (**Proposed**): экспонировать brain как **MCP-сервер** (tools explore/heal/replay/report), отдельный от MCP-сервера pw-executor; host (OpenCode/Kilocode/Claude Desktop) драйвит его и через MCP `sampling/createMessage` поставляет модель — реализуется как `SamplingBackend(LLMBackend)` поверх абстракции ADR-019 | Proposed | Закрывает второе направление запроса пользователя («работать из агентов-хостов»). Абстракция B1 (ADR-019) уже sampling-совместима: `SamplingBackend.supports_vision=False`, токены 0, `LLMResult.model` несёт реальную модель хоста, sync↔async мост как `McpExecutor`. Имплементация — следующий milestone (нужен живой MCP-host, user-run). **Отклонено:** делать B2 до B1 (sampling — частный случай backend, требует абстракции первой) |
