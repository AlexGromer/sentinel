...
| GAP-SEC-001 | P1 | **Полное наследование env + plaintext-секреты в Helm.** `agentctl` передаёт `cmd.Env = os.Environ()` без allowlist (cmd/agentctl/main.go:68) — каждый хост-секрет наследуется brain+детьми. Helm CronJob инжектит env как plaintext `value:` (deploy/sentinel/templates/cronjob.yaml:34–46, вкл. `CHECKPOINT_DSN` + `extraEnv`) — без `Secret`/`secretKeyRef`. | **ЧАСТИЧНО (этот цикл):** env-allowlist в agentctl доступен **opt-in** (`SENTINEL_ENV_ALLOWLIST=1` + `SENTINEL_ENV_ALLOW="NAME,…"` для secretRef-имён); default OFF — без слома прогонов. AUT-секреты уже через `secretRef`+`PW_NO_TRACE` (GAP-RISK-010). Остаётся: default-on после контейнерных тестов + Helm `secretKeyRef` — **M11.3**. Детали: docs/THREAT_MODEL.md |
