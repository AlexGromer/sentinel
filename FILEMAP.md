...
| docs/setup/index.html | setup-WebUI (ADR-031; M9.3 live) | static config generator (RunConfig YAML + env + command) + **download** buttons (full Pages generation) + **live mode** (control-API URL+token → /healthz → ▶Run/🔁Re-run/📌baseline POST /v1/runs → poll + RU-вердикт; M9.3 p2 + **M9.9/R1b** RU-only); vanilla JS, air-gapped; on Pages + Docker `webui` :8088 |
