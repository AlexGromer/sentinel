...
| GAP-OPS-002 | P2 | **AUT TLS certificate handling.** `browser.newContext` (pw-executor/src/server.ts:100) has no `ignoreHTTPSErrors` → an AUT with a bad cert (expired/self-signed/hostname-mismatch) yields a generic navigation error, not a classified one. | Goal: opt-in `PW_IGNORE_HTTPS_ERRORS` (strict by default) + cert-error classification in the report. Owner: **M9.4** |
| GAP-SEC-001 | P1 | **Full env inheritance + plaintext secrets in Helm.** `agentctl` passes `cmd.Env = os.Environ()` with no allowlist (cmd/agentctl/main.go:68) — every host secret is inherited by brain + children. The Helm CronJob injects env as plaintext `value:` (deploy/sentinel/templates/cronjob.yaml:34–46, incl. `CHECKPOINT_DSN` + `extraEnv`) — no `Secret`/`secretKeyRef`. | Goal: an env allowlist in agentctl + `secretKeyRef` plumbing in the chart. **Partially mitigated:** AUT secrets via `secretRef`+`PW_NO_TRACE` (GAP-RISK-010, MITIGATED). Owner: **M11.3**. Detail: docs/THREAT_MODEL.md |
