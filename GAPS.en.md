...
| GAP-M9-13 | P2 | **Action recorder (M9.8 extension):** content-script capture of click/input → events → grounding into steps (reuse M9.2b `reconcile`). Absent today (describe-mode is NL→scenario, not event-capture). Blocks M9.8 implementation | M9.8 (design-first contract ready — ADR-038); implementation after the M9.3 channel |
| GAP-M9-14 | P2 | **Streaming transport extension↔brain (M9.8):** the control-API (M9.3) was request/response only (no WS/SSE). A streaming endpoint (or native-messaging host) is needed. Blocks M9.8 | **PARTIAL (M9.3-tail, ADR-040):** the server→client **SSE** `GET /v1/runs/{id}/events` (token-gated, ring-buffer+fan-out) is implemented; the client→server recorder channel (WS *or* native-messaging) → **M9.8-impl**. reuses ADR-032 security |
| GAP-M9-15 | P2 | **Takeover/return protocol (M9.8):** handing control agent↔human on one CDP session (on top of M9.6). Undefined. Blocks co-pilot | M9.8 (ADR-039, design-first); implementation after recorder+transport |
