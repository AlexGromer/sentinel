...
| GAP-SEC-001 | P1 | **Full env inheritance + plaintext secrets in Helm.** `agentctl` passes `cmd.Env = os.Environ()` with no allowlist (cmd/agentctl/main.go:68) — every host secret is inherited by brain + children. The Helm CronJob injects env as plaintext `value:` (deploy/sentinel/templates/cronjob.yaml:34–46, incl. `CHECKPOINT_DSN` + `extraEnv`) — no `Secret`/`secretKeyRef`. | **PARTIAL (this cycle):** the agentctl env allowlist is available **opt-in** (`SENTINEL_ENV_ALLOWLIST=1` + `SENTINEL_ENV_ALLOW="NAME,…"` for secretRef names); default OFF — no run breakage. AUT secrets already via `secretRef`+`PW_NO_TRACE` (GAP-RISK-010). Remaining: default-on after container tests + Helm `secretKeyRef` — **M11.3**. Detail: docs/THREAT_MODEL.md |
