...
- [ ] [W2] M2b: extract Go store-gateway (sole SQLite writer, WAL) + gRPC + proto v1 (PersistenceService) to replace the interim brain-local store (ADR-012) and restore ADR-007 single-writer; migrate brain<->pw-executor transport to the official MCP SDK (GAP-VERIFY-002, @modelcontextprotocol/sdk server + python mcp client). (P1) @api-developer — 2026-06-22
