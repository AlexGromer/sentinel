...
const NOT_PRESSED = {
  ...
  'Сменить парольChange password': '`#pw-save` лежит в `#id-pw`, который показывается ТОЛЬКО вошедшему ' +
    'человеку (ADR-159): у пароля есть владелец, а этот обход держит МАШИННЫЙ токен, у которого аккаунта ' +
    'нет и менять нечего. Довести обход до входа значило бы сменить личность посреди протыкивания — ' +
    'дальше он мерил бы уже чужие строки, а не полноту разметки. ⚠ Контрол при этом НЕ непроверен: он ' +
    'нажимается в `scripts/hub-dom-check.mjs` («identity: a signed-in person changes their own password»), ' +
    'где вход выполняется, и там же утверждается, что старый пароль перестаёт пускать, а новый пускает',
};
...
// 5 -> 6: ADR-159 добавил смену своего пароля, и её кнопка видна только вошедшему, тогда как этот
// обход держит машинный токен. Причина записана рядом с исключением и называет ГЕЙТ, где контрол
// действительно нажимается, — потолок двигается вместе с решением, а не вместо него.
const MAX_NOT_PRESSED = 6;
