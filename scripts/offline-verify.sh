...
#       a v* release): MANIFEST checksums, Cosign bundles (offline, no Rekor call — the SET is carried
#       in the .cosign.bundle), then brings up the full offline stack and probes the local Ollama
#       /v1/models endpoint. Cannot pass until a v* tag exists (the GHCR image + signed archives are
...
COSIGN_MANIFEST_ID_RE="${COSIGN_MANIFEST_ID_RE:-.*}"                 # maintainer identity; override in CI/maintainer env
COSIGN_MANIFEST_ISSUER="${COSIGN_MANIFEST_ISSUER:-$COSIGN_ISSUER}"

# Optional buildx cache backend (CI passes AIRGAP_CACHE='--cache-from type=gha --cache-to type=gha,mode=max').
...
verify_local() {
  ...
  info "agentctl --help under --network none"
  docker run --rm --network none "$IMAGE" --help >/dev/null || fail "agentctl --help failed offline"
...
verify_bundle() {
  ...
  info "Cosign bundles verify offline (SET carried in the bundle — no Rekor network call)"
  local b f id
  for b in *.cosign.bundle; do
    [ -e "$b" ] || fail "no .cosign.bundle files found in $dir"
    f="${b%.cosign.bundle}"
    [ -f "$f" ] || fail "signed file for $b not found ($f)"
    if [ "$f" = "MANIFEST.sha256" ]; then id="$COSIGN_MANIFEST_ID_RE"; else id="$COSIGN_RELEASE_ID_RE"; fi
    cosign verify-blob "$f" --bundle "$b" \
      --certificate-identity-regexp "$id" \
      --certificate-oidc-issuer "$( [ "$f" = "MANIFEST.sha256" ] && echo "$COSIGN_MANIFEST_ISSUER" || echo "$COSIGN_ISSUER" )" \
  ...
  if [ -f ollama-models.tar.gz ]; then
    info "restoring the ollama model volume from the bundle"
  ...
  info "bringing up the offline stack + probing Ollama /v1/models"
  docker compose -f "$COMPOSE" --profile ollama up -d ollama || fail "offline stack failed to start"
  local ok=""
  for _ in $(seq 1 30); do
    if curl -sf --max-time 2 http://localhost:11434/v1/models >/dev/null 2>&1; then ok=1; break; fi
    sleep 2
  done
  [ -n "$ok" ] && pass "Ollama /v1/models responds offline" || info "WARN: /v1/models not reachable (model present?)"
  docker compose -f "$COMPOSE" --profile demo run --rm demo || fail "offline demo run failed"
