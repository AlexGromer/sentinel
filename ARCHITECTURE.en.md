...
| ADR-020 | 2026-06-25 | M7 (**Proposed**): expose the brain as an **MCP server** (tools explore/heal/replay/report), distinct from the pw-executor MCP server; a host (OpenCode/Kilocode/Claude Desktop) drives it and supplies the model via MCP `sampling/createMessage` — implemented as `SamplingBackend(LLMBackend)` on top of the ADR-019 abstraction | Proposed | Closes the second direction of the user ask ("be driven by host agents"). The B1 abstraction (ADR-019) is already sampling-compatible: `SamplingBackend.supports_vision=False`, tokens 0, `LLMResult.model` carries the host's real model, sync↔async bridge like `McpExecutor`. Implementation is the next milestone (needs a live MCP host, user-run). **Rejected:** doing B2 before B1 (sampling is a special case of a backend, needs the abstraction first) |
