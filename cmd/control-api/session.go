...
func (s *server) resolveCred(tok string) (caller, bool) {
	...
	// ADR-160: именованные машинные токены — ВТОРОЙ источник машинного кредентиала, и он попадает
	// сюда, потому что это единственное место сравнения: обе двери (заголовок и подпротокол
	// WebSocket) получают его даром, а разложить проверку по обработчикам значило бы завести правило
	// доступа в двух местах — та самая болезнь, от которой лечит таблица маршрутов.
	//
	// ⚠ ПОРЯДОК: после сервера, но ДО сессий. Именованный токен — это машина, и он обязан получить
	// те же права, что серверный (`machine: true` = незаскоуплен): CI, которому выдали собственный
	// токен, отличается от CI с общим только тем, что его можно отозвать поштучно, а не объёмом
	// видимого. Имя переносится в `caller`, чтобы журнал мог назвать, КТО из машин пришёл, —
	// невозможность этого и была одной из четырёх причин завести эту задачу.
	if name, ok := s.machineTokenMatches(tok); ok {
		return caller{machine: true, name: name}, true
	}
