...
- **Surfaces конфигурации (режим выбирается per-run, не live-тумблер):** (1) флаги `agentctl` → env
  (DH ConfigMap/Secret); (2) **RunConfig YAML** для богатых прогонов (goal-текст, auth, бюджеты, сценарии —
  не влезает во флаги); (3) **интерактивно через чат-UI** (M9.3) — описал цель словами, режим подразумевается.
  Live-переключения посреди прогона нет: run коммитится в режим. (GAP-M9-09)
