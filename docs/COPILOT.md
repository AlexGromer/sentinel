...
| **Replay/baseline ВНУТРИ UI** | M9.3 «вне scope» → **R1/M9.9** | ⏳ deferred → запланировано (GAP-M9-16) |
...
|---|------|-----------|-----------|
| **R1** | **M9.9 In-tool run-console** | control-API `mode=replay\|baseline` + `from_run:<run_id>` (whitelist+traversal-guard; `--replay --plan`/`baseline`) + `config-schema.modes`; ▶/🔁/📌 + вердикт в vanilla-UI (`#build`/`#chat`/`chat/`/`setup/`); httptest | GAP-M9-16 |
