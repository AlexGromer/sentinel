...
SHORT-TERM (episodic, within a single run):
  The LangGraph RunContext TypedDict IS the short-term memory. It lives in-process in the Python brain and is serialized by the LangGraph checkpointer after each significant node transition. Key episodic fields: action_history (list of ActionRecord trimmed to last 20 entries for LLM context window), messages (Annotated accumulator of BaseMessage for the LLM conversation thread), current sitemap being built, healing attempts counter per scenario. Trimming action_history to 20 prevents unbounded context growth on long exploration runs; the full history is available via the EventService stream in Go for audit purposes. **(The `messages` accumulator is ✅ built in M9.10/R2a — `RunState += messages: Annotated[list, add_messages]`, checkpointer-resume by `conversation_id`→`thread_id`; ADR-048.)**
