...
| **A video recording of a run** | observe=record writes `runs/<id>/video.webm`; fetched like any artifact (GET /v1/runs/{id}/artifact?name=video.webm, agentctl runs artifact) and played in the hub's artifact panel. ⚠ It carries the CURSOR, so it CHANGES TIMING and cannot be a golden; ⚠ impossible over CDP-attach — refused before the run starts. By default it is kept only when the run did not finish clean; SENTINEL_VIDEO_ALWAYS=1 keeps it always. The collector does NOT take it: pixels are not redactable. |
