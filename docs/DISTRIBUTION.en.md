...
| `browser-vnc` | `vnc` | **The real screen** (LIVE-VNC, ADR-127): the same Chromium, but HEADED, inside an Xvfb virtual display `:99` exported by `x11vnc` over RFB at `browser-vnc:5900`. The only mode with a real cursor and real input. The image is the SAME `sentinel` one: measured 2026-08-17, `x11vnc` adds **10 MB** (the X stack is already there for Chromium), so there is no second image. No `ports:` key: neither CDP (9223) nor RFB (5900) leaves the network — the hub reaches the screen through control-api's relay behind a bearer token. A password is MANDATORY: `SENTINEL_VNC_PASSWORD`, or the generated `state/vnc.password` (0600). ⚠ VNC authentication is DES over the FIRST EIGHT BYTES of the password (measured against x11vnc 0.9.16); anything longer is truncated by the protocol, not by us. ⚠ In the `ghcr` form the profile needs an image published AFTER ADR-127: an older one carries neither `x11vnc` nor the entrypoint, and the failure reads `exec /app/bin/vnc-entrypoint.sh failed: No such file or directory` — a message that is true and useless, naming a path rather than a reason (measured against `:latest` = v0.2.1). |
