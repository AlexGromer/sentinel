...
| `browser` | (всегда) | Chromium, переживающий прогон, доступный по CDP на `http://browser:9223` (ADR-110). Ключа `ports:` нет НАМЕРЕННО: у CDP нет аутентификации. Работает HEADLESS и таким остаётся: `screenshot_hash` байт-стабилен только в headless (`docs/DETERMINISM.md`). |
| `orchestrator` | (всегда) | Надзор RunControl (ADR-126) на unix-сокете `state/orch.sock`; control-api дозванивается по `CONTROL_API_ORCH_ADDR`. Без него МОЛЧА отсутствуют бюджетный потолок (ADR-021), takeover оператора (ADR-054) и map-гейт (ADR-108c) — прогоны при этом идут и завершаются, поэтому отсутствие не видно снаружи. ⚠ Строки в этой таблице не было со дня появления сервиса. |
| `browser-vnc` | `vnc` | **Настоящий экран** (LIVE-VNC, ADR-127): тот же Chromium, но HEADED, внутри виртуального дисплея Xvfb `:99`, экспортированного `x11vnc` по RFB на unix-сокет `state/vnc.sock`. Единственный режим с настоящим курсором и настоящим вводом. Образ — ТОТ ЖЕ `sentinel`: замерено 2026-08-17, что `x11vnc` добавляет **10 МБ** (X-стек уже стоит ради Chromium), поэтому второго образа нет. Ключа `ports:` нет, и у RFB порта НЕТ ВООБЩЕ: x11vnc слушает unix-сокет `state/vnc.sock` с `-rfbport 0` (замерено: `/proc/net/tcp` пуст). ⚠ Пароля тоже нет — и это решение о безопасности: классическая VNC-аутентификация — DES по первым восьми байтам над нешифрованным каналом, а слабых шифров мы не поставляем. Доступ решают ПРАВА СОКЕТА (0600; замерено: чужой uid получает `EACCES`), и реле control-api отказывается отдавать экран, если права шире. Хаб ходит через `GET /v1/live/screen` за bearer-токеном. ⚠ В форме `ghcr` профиль требует образ, выпущенный ПОСЛЕ ADR-127: в более старом нет ни `x11vnc`, ни entrypoint'а, и отказ выглядит как `exec /app/bin/vnc-entrypoint.sh failed: No such file or directory` — сообщение верное и бесполезное, оно называет путь, а не причину (замерено против `:latest` = v0.2.1). |
...
**Кто владеет тем, что там появляется (HEALTH-005 PR-C).** Сервисы нашего образа работают как
`${UID:-1000}:${GID:-1000}`, то есть файлы, созданные контейнером в этих томах, принадлежат
оператору, а не root. До этого `USER` в образе был пуст, контейнеры работали от root, и всё
созданное не убиралось без sudo (`rename(2)` каталога пишет в него самого, поэтому права на родителя
не помогают), ломало `go test ./...` на чекауте и сделало сервисный журнал нечитаемым с хоста.

⚠ **`UID`/`GID` — переменные ОБОЛОЧКИ, а не окружения.** Без `export UID GID` (или строк в `.env`)
подстановка молча даёт 1000 для всех. На хосте, где вы и есть uid 1000, это верно; на любом другом —
нет. Образец с объяснением: `.env.example`.

⚠ **Миграция, если стек уже поднимался.** Файлы, созданные прежними root-контейнерами, остались
root-owned, и store-gateway не сможет открыть свою базу под новым пользователем. Одна команда, один
раз:
...
**Ротация логов.** У всех сервисов всех трёх compose-файлов стоит `logging:` с `max-size: 10m` и
`max-file: 3`. До этого драйвер не был настроен вовсе: контейнер наследовал умолчание демона —
обычно json-file без ограничения размера, — и лог долгоживущего control-api рос, пока не кончался
диск. Сервисный журнал (`state/logs/service.jsonl`) переживает `docker compose down`, в отличие от
журнала докера; что в нём есть и чего нет — `docs/OBSERVABILITY.md` §8.
...
# Верифицировать подписи АРХИВОВ bundle (Cosign offline через bundle).
# Не подпись образа: `docker save`/`load` её не переносит (см. выше) — образ верифицируется на
# связанной машине ДО сохранения, а целостность bundle держит подписанный `MANIFEST.sha256`.
