...
| `browser-vnc` | `vnc` | **Настоящий экран** (LIVE-VNC, ADR-127): тот же Chromium, но HEADED, внутри виртуального дисплея Xvfb `:99`, экспортированного `x11vnc` по RFB на `browser-vnc:5900`. Единственный режим с настоящим курсором и настоящим вводом. Образ — ТОТ ЖЕ `sentinel`: замерено 2026-08-17, что `x11vnc` добавляет **10 МБ** (X-стек уже стоит ради Chromium), поэтому второго образа нет. Ключа `ports:` нет: ни CDP (9223), ни RFB (5900) наружу не выходят — хаб ходит через реле control-api за bearer-токеном. Пароль ОБЯЗАТЕЛЕН: `SENTINEL_VNC_PASSWORD` либо сгенерированный `state/vnc.password` (0600). ⚠ VNC-аутентификация — DES по ПЕРВЫМ ВОСЬМИ байтам пароля (замерено на x11vnc 0.9.16); длиннее обрезает протокол, а не мы. ⚠ В форме `ghcr` профиль требует образ, выпущенный ПОСЛЕ ADR-127: в более старом нет ни `x11vnc`, ни entrypoint'а, и отказ выглядит как `exec /app/bin/vnc-entrypoint.sh failed: No such file or directory` — сообщение верное и бесполезное, оно называет путь, а не причину (замерено против `:latest` = v0.2.1). |
