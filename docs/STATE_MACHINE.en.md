...
| Checkpoint DB path | `<artifact_dir>/checkpoint.db` — one SQLite file per run (assembled in `brain/__main__.py::_run_explore`), never the store-gateway file |
| Thread identity key | `thread_id = run_id` |
| Production DB (`CHECKPOINT_DSN`) | Synchronous `PostgresSaver` (package `langgraph.checkpoint.postgres`) replaces `SqliteSaver` when `CHECKPOINT_DSN` is set — same interface, schema unchanged (`brain/__main__.py::_checkpointer`) |
...
> on `run_mode` inside the graph (graph assembly in `brain/graph.py::build_graph`: edges are either unconditional or routed by
...
> dispatched from `_run_replay()` (`brain/__main__.py`). The real locator-healing engine
...
`RunState` is the single shared object threaded through every node (`brain/state.py::RunState`,
`TypedDict total=False`). All fields, except the internal `_`-channels, are checkpointed
...
| `chat_intent` | **ADR-108a** — the conversation's OBJECTIVE, pinned on the first turn and never rewritten: `{"kind": "goal"\|"describe", "text": str}`. It exists because `goal` was doing two jobs at once: control-api sent EVERY turn's text as the goal, so "what this conversation is for" and "what the person just typed" were the same field, and nothing could tell a refinement from a new objective. It lives in the checkpointer (`thread_id=conversation_id`), NOT in the `chats` SQL row, whose `last_goal` column keeps meaning exactly what its name says: the most recent turn |
| `site_map` | Site-wide element map `page_path -> [element]`, accumulated by `ground` over the whole explore walk |
| `perception` | **ADR-092/093/097** — `page_path -> {seen, total, ratio, unseen{…}, opaque{…}}`: how much of the page perception can SEE, as opposed to coverage (how much of what it saw was exercised). Filled by the `ground` node (a `browser.perceptionAudit` call) once per page; the `report` node puts `perception.pages` and `worst_ratio` into `plan.json` BESIDE `steps`, so `plan_hash` does not move. There is still no `completeness_ratio` field in the code — this is a different, genuinely existing measure under a different name |
...
| `page_model` | Dict assembled by `perceive`/`ground`: `{url, title, aria, nodeCount}` plus `buttons` (added by `ground`). **Does not** contain `a11y_tree`/`landmarks`/`forms`/`completeness_ratio` or any hashes — the code never computes those subfields (`brain/graph.py::perceive` / `::ground`) |
...
The graph has **10 named nodes** (`brain/graph.py::build_graph`, the `b.add_node` loop: `perceive`, `ground`, `plan`, `act`,
...
- Catalogues interactive elements (`brain/graph.py::_elements_from_interactives`): role, name,
...
- Unconditional edge `ground → plan` (`b.add_edge("ground", "plan")` in `brain/graph.py::build_graph`) — no branching on `run_mode`.
...
in `brain/__main__.py::_run_explore`).
...
- Route (`brain/graph.py::route_verify`): `"checkpoint" if ok else "heal"` — exactly 2 outcomes.
...
- There is **no** `store-gateway` write here, **no** `PersistenceService` call, **no** reset of
  ...
  node's code.
- Route (`brain/graph.py::route_checkpoint`) — exactly 3 targets: `exploration_complete →
...
- `app.invoke()` returns control with `__interrupt__`; the live browser is handed to the operator
  ...
  the same point (`brain/__main__.py::_resume_through_takeovers`).
...
- **M9.10 (ADR-048): also the RESUME entry point for a warm multi-turn conversation.**
  `brain/graph.py::route_entry` routes `START` straight here, bypassing `perceive`, when the
...
(`brain/__main__.py::_run_replay`) calls `run_replay()` (`brain/replay.py::run_replay`) directly — a plain Python
...
- **Per step:** `navigate`/`assert`/`press` execute directly; for `click`/`fill`/`type`/`select`,
  ...
  away, otherwise the **real** `HealingEngine.heal(ctx)` is called (`brain/healing.py::HealingEngine.heal`):
  ...
  2. Strategy rotation over the recorded `alternatives`: the first locator that resolves to
     exactly one element (`brain/strategies.py::PRIORS`, ADR-083; `healing.py` only re-exports it):
     ...
     | `css` | ONLY from foreign plans — the importer (`brain/importer.py`) and the recorder (`brain/record_bridge.py`, where the strategy is derived from the observed locator's key). Since ADR-082 the LLM no longer authors it | 0.65 (rotation calls `prior_for`, which applies no discount) |
     ...
     | `llm_pick` | the ONLY strategy the LLM tier produces today (step 3 below, ADR-082) | the prior of the RESULTING locator TYPE × `PICK_DISCOUNT` 0.90 ⇒ 0.855 (`testid`) or 0.81 (`role_name`) |

  3. If rotation found nothing — LLM re-grounding (`_llm_reground`): since ADR-082 the model does
     NOT author a selector, it picks an INDEX in the list the executor already reported
     (`browser.interactives`); an out-of-range index is discarded, and the chosen descriptor is
     turned into `{testid}` or `{role,name}`. The resulting strategy is `llm_pick`. It runs only if
     an LLM backend is wired (`use_llm=True`, typically `HEAL_LLM=1`) and the `heal` budget is not
     exhausted (`budget.tracker().exceeded("heal")`).
  4. If that found nothing either — visual set-of-marks (`_visual_reground`), only if an LLM backend
     is wired at all (`use_llm=True`, `HEAL_LLM=1` — otherwise `self._backend` is empty and the tier
     is never called), `use_visual=True` (`HEAL_VISUAL=1`) **and** the backend supports vision; the
     `heal` budget is checked once more inside the tier. There is no `completeness_ratio` gate —
  ...
  6. Gate: `confidence >= AUTO` → `auto_healed` (locator persisted as `active`); `FLAG..AUTO` →
     `flagged` (applied optimistically and reported); `< FLAG` → `needs_review`, the locator is NOT
     persisted, the step fails. `AUTO`/`FLAG` are not constants: 0.85/0.60 are DEFAULTS, overridable
     via `SENTINEL_HEAL_AUTO`/`SENTINEL_HEAL_FLAG` (ADR-080). Two more mechanisms sit between the
     confidence and the outcome: the `_cap` cap (a RE-GROUND that reaches `AUTO` is cut to
     `(AUTO+FLAG)/2` and can never reach AUTO) and the ADR-082 identity predicate.
  7. Every attempt writes a row to the `healing_audit` SQLite table (append-only,
     `brain/store.py::LocalStore.audit`) — no `UPDATE`/`DELETE` ever.
- **Flake quarantine:** `store.record_step(plan_id, step_key, passed, aut_version)` keeps a
  ...
  quarantine (`brain/store.py::LocalStore.record_step`; in Go, `internal/store/server.go::RecordStep`). Golden-diff regressions (`exit 2`) are NOT suppressed by
  quarantine.
...
- **AG-UI + auto-HITL (M14 tail 2, ADR-055):** emits `run.started`/`step.progress`/`heal`/
  ...
  threshold as the graph, `brain/replay.py::run_replay`) and emits `hitl_needed` at the threshold — but
...
Source of truth: `brain/graph.py::build_graph` (`b.add_edge`/`b.add_conditional_edges`). Every edge
...
Exactly 4 functions emit conditional edges; none reads `run_mode` (`brain/graph.py::build_graph`: `route_plan`, `route_verify`, `route_checkpoint`, `route_entry`):
...
> Simplified rendering of the real edges in `brain/graph.py::build_graph` (see the precise table in §4.1).
