...
TypeScript Playwright test-файл, генерируемый `report-service` из
`RunState.executed_actions` через Go-шаблон.
...
генерируется из записанных данных о действиях в Go-слое, а не путём запроса к `pw-executor`
на испускание кода. Это устраняет хрупкую зависимость от MCP, отмеченную архитектурными
ревьюерами.
