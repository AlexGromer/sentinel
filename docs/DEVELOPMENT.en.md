...
5. **A list is DERIVED, never maintained.** A check may not keep its own list of what it must cover — view names, service sets, module sets. The reason is not effort but the way such a list FAILS: an extra entry is visible (a row for something deleted breaks the run), a MISSING one is not, because absence has no representation to look at. Measured: the UI smoke took screenshots from a list of views written inside itself; when a new view was added it was appended, and nobody noticed that two others had never been there. So: views come from the hub's own `VIEWS`, services to probe and log from the compose files, routes to audit from a walk of `routes()`. **A FLOOR is mandatory alongside** ("at least nine views", "at least thirty routes"): a walk that has stopped finding anything passes perfectly over an empty set, and that is the one thing deriving cannot catch by itself. ⚠ A tree walk states its exclusions EXPLICITLY: one `uv sync` from the wrong directory turned a walk of 37 files into a walk of 4733 and had the check demanding things of third-party packages.
6. **Every capability is reachable THREE ways** — from the UI, from a terminal, over HTTP — or the missing path is recorded with its reason. `docs/capabilities.json` must carry ALL paths per entry, not one: while it carries one, it answers "at least somewhere" and is read as "covered", and the principle has nothing to measure it with.
7. **A PR that adds a component or a feature MUST extend the observation of it** (Alex's directive 2026-08-07, in force from the close of HEALTH-006). Specifically, and without exception: a health probe and logging for the new component · a row in the component registry the probe list is derived from · an entry in `docs/capabilities.json` with all of its access paths · coverage in the [verification matrix](PR_ACCEPTANCE.en.md) (offline suite, DOM gates, screenshots, a live run, docker in EVERY delivery shape, mutations). The reason is that observation added "later" is not added: the component reaches the stack, works, and its absence from the probes is discovered on the day it falls over. A component with no probe is a component the deployment reports as healthy by default. A missing item requires a RECORDED reason next to the code, not silence: the gates derive their lists from the deployment's own composition and from the route table, so a new component enters them by construction and cannot be silently skipped.
...
```bash
# offline suite (no network/binaries): the whole offline suite, DISCOVERED by glob.
# The count is not written here — it goes stale faster than it gets fixed: `ls tests/test_*_offline.py | wc -l`
