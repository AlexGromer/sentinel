...
- **replay/baseline AG-UI + auto-HITL — ✅ эмиссия+сигнал (M14 tail 2):** `run_replay` теперь эмитит `run.started`/`step.progress`/`heal`(реальные L1-L6 strategy/confidence)/`verdict`(реальный exit 0/1/2/3) — богатый timeline вместо `log`-view; счётчик consecutive-heal-failures + `hitl_needed` на `SENTINEL_AUTO_HITL_THRESHOLD` (0=off). **Живая auto-ПАУЗА (человек берёт управление посреди replay) = M9-LIVE** — у replay нет interrupt/resume-механизма, а co-pilot-takeover-in-replay отложен туда же (как и живая проверка graph-mode-паузы). `tests/test_m14_replay_agui_offline.py`.
