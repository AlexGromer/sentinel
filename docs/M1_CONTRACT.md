...
> ⚠ **ADR-139: «≥5 шагов» — утверждение о ЭТОЙ ФИКСТУРЕ, а не критерий кода выхода.** До этой волны
> порог стоял в `_run_explore` и применялся к ЛЮБОЙ цели, поэтому наказывал размер приложения:
> замерено, что одностраничный сайт с одной кнопкой, обойдённый ЦЕЛИКОМ (`converged`, coverage 1.00),
> получал `exit 1`, то есть `fault: app` «тест нашёл проблему» над прогоном без единой проблемы.
> Из кода порог убран; здесь он остаётся тем, чем и был задуман, — приёмочным свойством фикстуры
> `testdata/site`, и впервые ИСПОЛНЯЕТСЯ полом в офлайн-сьюте, а не только косвенно через `set -e`
> над `agentctl run` в CI.
