...
> **Новые поверхности (M9.6/M9.8/M9-LIVE-prep), не показанные на диаграмме выше (опциональны/будущее):** ❽ **CDP-attach** к браузеру пользователя (M9.6, opt-in `PW_CDP_ENDPOINT`), ❾ **браузерное расширение** (M9.8, планируется) и ❿ **экспорт live-run артефактов** (`scripts/collect-live-run.sh`, M9-LIVE-prep) — см. §4.8 / §4.9 / §4.10.
>
...
### 4.9 Граница ❾ — браузерное расширение (M9.8, ЧАСТИЧНО РЕАЛИЗОВАНО)

> **MV3-расширение само ещё не реализовано** (отдано @0xCoDSnet, #47) — но транспорт (WS `/v1/stream`, ADR-043) и **brain-side takeover/return** (R3, ADR-054) уже РЕАЛИЗОВАНЫ, поэтому строки транспорта и takeover-мутации ниже — «done», а не «планируется». Крупнейшее расширение surface проекта: MV3-расширение, живущее в браузере пользователя.
