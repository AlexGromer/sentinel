...
### 4.17 Граница ⓱ — экран как сервис: RFB-порт и пароль на диске (LIVE-VNC, ADR-127)

> Поверхность появляется вместе с профилем `vnc` и при выключенном профиле НЕ СУЩЕСТВУЕТ: не
> поднимается `browser-vnc`, не создаётся `state/vnc.password`, `CONTROL_API_VNC_LIVE` пуст, и проба
> `/readyz` отвечает `skipped`. Отличие от границы ⓰ не в масштабе, и это надо сказать прямо: там
> наружу шли КАДРЫ под учётными данными control-API, здесь наружу идёт РАБОЧИЙ СТОЛ, и он **принимает
> ввод**. Второе не является усилением первого — это другая буква STRIDE.

| Угроза (STRIDE) | Вектор | Текущая защита | Остаточный риск |
|---|---|---|---|
| **S/E** — чужая мышь в живом сеансе | Кто дошёл до RFB-порта и знает пароль, получает клавиатуру и мышь в браузере, уже залогиненном в тестируемое приложение под cookies пользователя. Это не «посмотреть»: это takeover, живой с первого дня по решению Alex. Оттуда достижимо всё, что достижимо человеку за этим браузером, и в журнале ТЕСТИРУЕМОГО приложения это будут действия пользователя, а не робота | Пароль ОБЯЗАТЕЛЕН: `-nopw` не встречается ни в одном файле поставки, и это утверждает гейт (`tests/test_vnc_profile_offline.py`), а не обещание в комментарии. `scripts/vnc-entrypoint.sh` работает под `set -e` и запускает x11vnc только после того, как `agentctl vnc-password` вернул 0; верб возвращает **2**, когда пригодного пароля нет ни по одному из двух путей. Порт НЕ публикуется: у `browser-vnc` нет ключа `ports:`, и проверка ВЫВОДИТ перечень сервисов, к которым правило применяется, а не перечисляет имена | Классическая VNC-аутентификация проверяет **первые восемь байт** пароля и не шифрует канал: содержимое экрана и нажатия клавиш идут открытым текстом. Замерено 2026-08-17 на x11vnc 0.9.16: сервер с паролем `ABCDEFGH12345678` принимает `ABCDEFGH` и отвергает `ABCDEFGX`. Внутри docker-сети граница ровно та же, что у CDP-порта (§4.16): «кто дошёл до docker-сети ЭТОЙ машины», а не «кто дошёл до localhost». На общем/многопользовательском хосте так ставить нельзя |
| **I** — пароль на диске | `state/vnc.password` — долгоживущий секрет **в открытом виде**, в каталоге, примонтированном во все контейнеры (`./state:/app/state`) и лежащем на хосте | Запись атомарная (temp+rename), `0600` с момента создания (`os.CreateTemp`), каталог `0700`; `state/` в `.gitignore`. Существующий, но непригодный либо нечитаемый файл НИКОГДА не перезаписывается — процесс уходит на одноразовый пароль в памяти и говорит об этом, называя путь. Правило и его причины те же, что у `state/control-api.token` (§4.11) | **Открытый текст — осознанно.** Обфусцированный формат `-rfbauth` шифрованием не является (фиксированный ключ DES, разворачивается публично известным способом): он не дал бы ни бита конфиденциальности, зато лишил бы оператора возможности прочитать собственный пароль и сломал бы правило «не затирать непригодное» — бинарный блоб непригоден ВСЕГДА, то есть пароль не пережил бы ни одного рестарта. Шифрования at-rest нет — тот же уровень, что у `state/control-api.token` и `state/golden.key`. На Windows `0600` ложится на ACL-семантику |
| **I** — рабочая копия пароля | x11vnc читает пароль из файла, и этот файл — ВТОРАЯ копия секрета | Копия пишется в `/tmp` внутри контейнера, а НЕ в `./state`: она не попадает на хост и умирает вместе с контейнером. Утверждается гейтом (`test_the_password_working_copy_never_lands_on_the_host_mount`) | Кто может выполнить команду внутри контейнера, прочитает и её, и `state/vnc.password` — но такой доступ уже равносилен доступу к браузеру |
| **E** — переменная расползается по процессам | `SENTINEL_VNC_PASSWORD`, экспортированный в шелле оператора, попадал бы под allow-ПРЕФИКС `SENTINEL_` в `cmd/agentctl/main.go::filteredEnv` и уехал бы в brain, в pw-executor и в Chromium, то есть в `/proc/<pid>/environ` | Deny-множество в `filteredEnv`, проверяемое ПЕРВЫМ — раньше `exact` и раньше `SENTINEL_ENV_ALLOW`. Та же форма исправления, что в #25 после утечки `NODE_AUTH_TOKEN`. В compose переменная объявлена ТОЛЬКО в собственном блоке `browser-vnc` | `SENTINEL_ENV_ALLOWLIST=0` (полный passthrough) обходит deny-множество: это документированная аварийная дверь §4.1, а не забытый путь |
| **T** — X-сервер как вторая дверь | Xvfb мог бы слушать TCP, и тогда к тому же дисплею вёл бы второй путь, у которого нет даже восьмибайтового пароля | `-nolisten tcp`: у X-сервера нет сетевого сокета вовсе, x11vnc ходит к нему через `/tmp/.X11-unix/X99` в том же контейнере. Утверждается гейтом | — |
