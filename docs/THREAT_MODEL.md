...
| **Страничный журнал маршрутов в MAIN world (ADR-138).** Журнал живёт в мире СТРАНИЦЫ и толкает `{__sentinel, url, how, main}` через `window.postMessage`; страница может подделать такой конверт. Плюс `expected` записанного сценария теперь содержит query-часть маршрута. | page → extension → артефакт | **S/I** | Вер: L / Влияние: M | Два обязательных гейта на приёме: `event.source === window` (замерено истинным для MAIN→ISOLATED в Chromium) и точное имя конверта. **Класс угрозы НЕ новый:** страница и сегодня может подделать `click` через `dispatchEvent`, который рекордер ловит в capture-фазе, — то есть подделка события записи возможна была всегда. **Редакция адреса обязательна:** значение query-параметра с секретным именем заменяется на `REDACTED` в единой воронке `emit()`, для ВСЕХ строк — иначе токен уезжает в `scenario.json`, который человек коммитит в репозиторий своего приложения. ⚠ Замерено попутно: ДО ADR-138 секрет в адресе попадал в `runs/record-<session>/events.ndjson` побайтово. | Остаточно: два маршрута, различающиеся только секретным параметром, становятся неразличимы — сознательный размен против записи секрета. | **ADR-138 DONE** |
...
### 4.18 Граница ⓲ — учётные данные провайдера, хранимые ИНСТРУМЕНТОМ (ADR-146)

> **Граница новая, и появилась она вместе с разворотом посадки.** До ADR-146 продукт заявлял
> инвариант «an api_key never travels through the API or UI» (`cmd/control-api/llmenv.go`), и
> следствием было отсутствие способа задать ключ вообще: мастер печатал `export LLM_API_KEY=...`
> ПЛЕЙСХОЛДЕРОМ, поля ключа не существовало, а `docs/index.html:3231` прямо сообщал «No api_key —
> secrets stay in the server env». Директива Alex (2026-08-07, 2026-08-09) требует, чтобы ключами
> управлял ИНСТРУМЕНТ, и чтобы ОБА пути хранения существовали одновременно.
>
> ⚠ **Ключ НЕ входит в домен конфига, и это и есть содержание границы.** Домен конфига защищён
> `internal/configguard`, чей собственный комментарий пакета называет причину: сокет store-gateway
> «reachable by any same-UID process holding STORE_TOKEN». Персист учётных данных за этой границей
> ОБОШЁЛ бы гейт, а не ослабил его. Поэтому ключ лежит отдельным файлом `state/provider-keys.json`
> 0600, атомарной записью temp+rename, и `configguard` не тронут: `Secretish` сохраняет три вызова
> (два из них — редакторы `internal/redact`, ослаблять которые нельзя), `Validate` по-прежнему
> отказывает секрет-образным членам в конфиге, обе точки принуждения целы.

| Угроза | Граница | STRIDE | Оценка | Митигация | GAP | Статус |
|---|---|---|---|---|---|---|
| **Ключ возвращается тому, кто его записал.** Форма, которой боялась запись реестра: значение снова живёт в DOM и в буфере обмена, и всякий, кто откроет вкладку, получит его. | UI/API → администратор | **I** | Вер: M / Влияние: H | **MITIGATED:** чтение только вперёд. Ни один маршрут не возвращает значение; `providerKeyStatus` не имеет члена, способного его нести, поэтому позднейшая правка не утечёт ключ, заполнив поле. Наружу — `set`, `from_env` и последние 4 символа при длине ≥12 (у короткого значения подсказки нет вовсе: четыре символа из шести — это само значение). Гейт ищет канарейку в СЫРЫХ байтах ответа, а не сверяет поля | — | ADR-146 ✅ |
| **Ключ попадает в argv и в историю команд.** `--value sk-…` видит любой `ps` на хосте. | CLI → процесс | **I** | Вер: H / Влияние: H | **MITIGATED:** `agentctl provider-keys set --value-stdin` — значение читается со stdin через существующий `apiVerb.SecretField`, ровно тот механизм, который `users add --password-stdin` уже завёл по этой же причине. Форма тела `{name, value}` выбрана ИМЕННО ради этого: у карты «имя→значение» нет фиксированного члена, который `SecretField` мог бы назвать | — | ADR-146 ✅ |
| **Хранимый ключ молча подменяет ключ хоста.** Развёртывание пробрасывает `LLM_API_KEY` с хоста, администратор сохраняет свой через UI — и прогоны уходят с ДРУГИМ ключом, а на экране написано «задан». | окружение процесса ↔ хранилище | **T** (Tampering) | Вер: M / Влияние: H | **MITIGATED:** хранимое — слой 4, САМЫЙ НИЖНИЙ в `resolveRunEnv`; `set()` не перезаписывает непустое значение, поэтому окружение процесса выигрывает всегда. Это же и гарантия air-gapped/CI. Сверх того признак `from_env` сообщается в интерфейсе отдельной строкой: администратор видит, что хранимое ПЕРЕКРЫТО, а не просто «задано» | — | ADR-146 ✅ |
| **Ключ утекает в один из каналов наблюдения:** сервисный журнал, лог прогона, `trace.zip`, артефакты, экспорт `collect-live-run.sh`. | процесс → артефакты | **I** | Вер: M / Влияние: H | **MITIGATED (проверяется живьём, а не чтением):** значение не проходит ни через домен конфига, ни через тело прогона (`llmRunConfig` по-прежнему без `api_key`), а до подпроцесса доезжает только переменной окружения — тем же путём, каким ключ хоста доезжал всегда, и под теми же редакторами `internal/redact` (ADR-098) | — | ADR-146 ✅ |
| **Ключ заменён не тем, кем следует.** «Живёт, пока АДМИНИСТРАТОР не заменит» — это решение о доступе. | маршрут → плоскость идентичности | **E** (Elevation) | Вер: M / Влияние: H | **MITIGATED:** оба маршрута объявлены `accessAdmin` в таблице `routes()` (ADR-109), то есть машинный токен или админский аккаунт; чтение тоже админское — состав учётных данных развёртывания не тот факт, который вправе перечислять каждый аккаунт. Гейт ВЫВОДИТ проверку из таблицы, а не повторяет её | — | ADR-146 ✅ |
| **Испорченный файл перезаписан, и ключ, копии которого нет, потерян.** | процесс → диск | **D** (DoS) | Вер: L / Влияние: M | **MITIGATED:** нечитаемый документ не перезаписывается — `PUT` отвечает 409 и НАЗЫВАЕТ путь; `GET` сообщает `readable: false`, а не «не задан» (второе читается как «ничего не сохраняли» и приглашает записать поверх) | — | ADR-146 ✅ |
