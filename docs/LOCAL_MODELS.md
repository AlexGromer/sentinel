...
| Контекст входа | ≤ **8000** симв. (PLAN-меню) / ≤ **3000** симв. (HEAL) ≈ ≤ 2000 / 750 tok | `planner.STEP_MENU_CHARS` · `healing.HEAL_MENU_CHARS` |

⚠ **Эти два предела стали правдой только с ADR-136.** До него у PLAN-меню кепа не было ВОВСЕ — документ обещал 8000 символов, а промпт шага нёс весь фронтир целиком (замерено: при фронтире в 1000 адресов — 131 562 символа, ×16 против обещанного). У HEAL кеп был, но резал уже сериализованную строку посреди JSON. И это границы ОДНОЙ строки промпта, а не промпта: фиксированный текст, `intent` и `attempted_locator` дают ещё около килобайта сверх.
...
Ниже — **рецепт** подключения локальной модели, а не реестр: полный выводимый перечень
переменных живёт в [`PARAMETERS.md`](PARAMETERS.md). Читает их `make_backend`.
**Приоритет: role-specific `LLM_<KEY>_<ROLE>` > глобальный `LLM_<KEY>`.**
Роли: `PLANNER`, `HEAL`, `CHAT` — ⚠ третья реальна с ADR-108b, и её отсутствие в этой строке было
ровно тем протуханием, ради которого заведён выводимый перечень. Ключи: `BACKEND`, `MODEL`,
...
### 8.4 Что «stable» НЕ означает (замерено 2026-08-08)

Классификация оснастки называется `stable_in_this_invocation`, и слово выбрано после замера, а не
из осторожности. Одна фикстура, один набор флагов, одна модель `qwen3:8b`, живая ollama:

| что запускалось | `plan_hash` |
|---|---|
| два прогона ВНУТРИ одного запуска | `df271e5a3bee` оба раза |
| три ОТДЕЛЬНЫХ запуска | `29abe04911b5` · `df271e5a3bee` · `906c6715…` |

То есть согласие наблюдается внутри запуска, а воспроизводимости между запусками нет. Причина в
коде, а не в модели: ни оснастка, ни `brain/llm.py` не передают `seed`, а `canonical_plan_hash`
(`brain/state.py:96`) хеширует ВЕСЬ упорядоченный список шагов — любая вариация выбора модели меняет
его целиком.

Практический вывод для `MODEL-001`: критерий «совпадает `plan_hash`» между моделями предъявлять
нельзя, пока выборка не закреплена. Сравнивать имеет смысл множество заземлённых шагов, а хеш
использовать как признак изменения порядка, и это ровно то различение, ради которого в оснастке два
показателя, а не один.
