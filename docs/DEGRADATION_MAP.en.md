...
| **pw-executor** (inside a run) | browser steps | a step error classified `tool`; ⚠ **a TOOL failure no longer takes the whole run with it** (ADR-131): what was found is written — a partial `plan.json` and `site-map.json` plus a proposed scenario — and the artefact declares itself incomplete through `completeness.reason: aborted` carrying the error text. The exit code is **5** (`fault: tool`), not 4 | control-api and the next run; all the crawl's work up to the point of failure | ❌ no probe of its own: it lives inside the run's process rather than as a service | it writes `service.started`/`stopped` under the browser service's name; on salvage, `explore.salvaged` (`warn`) |
