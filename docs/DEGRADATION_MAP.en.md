...
| **the browser dies MID-run** | the current run | `Target page, context or browser has been closed` plus the explicit caveat "This is NOT a finding about your app". **In `explore` mode it is `exit 5` when what was found got written, and `exit 4` when it did not** (ADR-131); `replay`/`baseline`/`chat` have no salvage and still return **4** | the next run opens its own | ✅ | — |
| **the executor answers but SEES NOTHING** | the whole crawl | `explore.saw_nothing` — "the crawl saw NOTHING on the target: not one element and not one address". **exit 4** (`fault: tool`), not 1: WE broke, and there is nothing to say about the application (ADR-139) | nothing: there is no artifact | ✅ a catalogue code with fault `tool` | ⚠ A LEGITIMATE page with no interactive elements exits **0**, not 4 — the ADDRESS separates them, and nothing else does (measured) |
...
| **agentctl** (the run's executable) | ALL runs | `POST /v1/runs` is still accepted with 202 — the failure happens after the answer, inside a goroutine — but the run is recorded `state: failed` with `exit_code: -1` and `fault: tool`, and the Health view shows `FAILING` with the path. Measured 2026-08-22 against `/no/such/binary` | the UI, the journal and the list all answer; **but the deployment no longer calls itself healthy** | ✅ **503 `not_ready`**, `agentctl: error` naming the path and what is wrong with it (`does not exist` / `is a directory, not a program` / `mode -rw-r--r--`) — ADR-129 | ✅ `service.run_spawn_failed` (`warn`) with the run id, the path and the failure text — ADR-129 |
...
| **browser-vnc** (`vnc`) | the "Screen" mode, the real cursor, taking the mouse | the Screen tab prints the reason and the command: «the screen is unavailable in this deployment: no VNC screen configured (CONTROL_API_VNC_SOCK is unset)» + `docker compose --profile vnc up -d browser-vnc` | every other live-area mode; the Video tab shows the headless browser | ✅ `vnc: skipped` with a reason. ⚠ **Measured 2026-08-22: `skipped` also arrives while the service is UP and `healthy`** — no compose file sets `CONTROL_API_VNC_LIVE`, so the probe sees an empty address. Since ADR-129 the reason no longer asserts «the profile is not running» (the probe cannot know that) and names both possibilities; the real fix is `[VNC-UP-BUT-UNWIRED]` and is NOT done | — |
| **webui** | serving the hub's pages on 8088 | ⚠ **the product says nothing**: the browser shows a connection error and that is all — no reason, no hint. Measured: before stopping it `webui:8088` → **200** with the hub markup, after → `ENOTFOUND` | control-api and all its routes (`/healthz` 200, `/readyz` unchanged), `agentctl` in full; control-api serves the pages itself when `CONTROL_API_SERVE_UI=1` | there is no probe — and since ADR-129 that is **served**, not merely declared: `/readyz` carries the component under the `unprobed` key with its recorded reason (three of them — in modes 1 and 3 there is no webui process at all (ADR-064), in mode 2 it is a stateless static file server, and a browser reaching it does not go through this API), and the Health view shows it as a «no probe» row. A token-bearing caller sees the reason; an anonymous one sees the name | — |
| **pw-executor** (inside a run) | browser steps | a step error classified `tool`; ⚠ **a TOOL failure no longer takes the whole run with it — IN EXPLORE MODE** (ADR-131): what was found is written, a partial `plan.json` and `site-map.json`, and a scenario is proposed only if the run was authoring one at all (`--goal`/`--describe`). The salvage sits around the crawl's `app.invoke`; `replay`, `baseline` and `chat` do NOT have it and still return their own code with no partial artefacts — said here rather than left to be inferred, because the degradation map is exactly where a reader goes looking for that difference — and the artefact declares itself incomplete through `completeness.reason: aborted` carrying the error text. The exit code is **5** (`fault: tool`), not 4 | control-api and the next run; all the crawl's work up to the point of failure | ❌ no probe of its own: it lives inside the run's process rather than as a service | it writes `service.started`/`stopped` under the browser service's name; on salvage, `explore.salvaged` (`warn`) |
