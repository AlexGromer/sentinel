...
1. Запусти базовый набор, сними метрики GAP-RISK-002 (confidence и доля unmatched на локальных моделях).
...
Это основной путь: прогоны настраиваются и запускаются из браузера, шаги видны на живом timeline. Обе фазы ниже выполняются так. Рекомендуемый на Windows-хосте режим (ADR-064) - single-service: один порт, ноль CORS-настройки, ноль copy-paste токена. Нужен один сервис `control-api` - в этом режиме он же отдаёт и HTTP API, и страницы UI, слушает `127.0.0.1:8090`.

Запусти сервис в режиме single-service. Токен доступа генерируется автоматически при первом старте - задавать его вручную не нужно; LLM-подключение по-прежнему задаётся в самом UI (ADR-063):
```powershell
...
Открой хаб `http://localhost:8090/`. Это co-pilot (ADR-055, ADR-066): слева вертикальное меню, один вид на экране. Состав меню задаёт рельс в `docs/index.html` (массив `VIEWS`) — если он и перечень ниже разойдутся, прав рельс. **Чат** — опиши проверку словами; **Новый прогон** — форма запуска (то, что раньше называлось «конфигуратор» и лежало под Settings); **Live** — живой таймлайн; **Библиотека** — сценарии, тесты, прогоны, беседы; **Результаты** — вердикты и метрики; **Логи** — что произошло в прогоне, по-человечески и с фильтрами; **Журнал сервиса** — что делал сам инструмент: старты и остановки, входы и неудачные входы, правка настроек, отказы в доступе; **Здоровье** — состояние компонентов этого развёртывания по данным `/readyz`, включая пробу LLM-эндпоинта; **Инструменты** — калькуляторы и справка; **Настройки** — подключение (адрес и токен, единственное место в продукте), LLM и уровни логирования. Внизу меню — точка состояния связи и версия. Прямая ссылка на любой вид: `#v=chat`, `#v=logs` и так далее.
...
| 4 | tool failure | Сломались МЫ (ADR-087) — не находка о приложении |
| 5 | tool failure, salvaged | Сломались мы, но найденное записано (ADR-131) |
| -1 | did not start | ⚠ Синтетический код control-api, процессом не возвращается |
...
Что проверяет: ARIA-табы, асинхронную инъекцию контента (через 600 мс) и элементы в shadow DOM (GAP-RISK-005).
...
Ожидаемый результат: если локатор сломался, в записи прогона (`heal-report.json`) видна стратегия L1-L6 и confidence; при успешном восстановлении вердикт 0. Вывод: детерминированный heal чинит сломанный локатор, срабатывает confidence-gate (данные для GAP-RISK-002).
...
```
На локальных моделях 14B и 7B доля FLAG и unmatched выше, чем на облачных Opus и Sonnet: пороги AUTO 0.85 и FLAG 0.60 настраивались на облачных моделях. Это ожидаемые данные для калибровки GAP-RISK-002 и GAP-RISK-003, а не ошибка. Записывай по каждому запуску в файл `runs\LIVE_NOTES.md`: идентификатор, модель, мишень, ожидаемый и полученный результат, код возврата, замеченные отклонения.
...
⚠ Логи control-api передавать не нужно и не следует: в режимах 1-2 он печатает значение `CONTROL_API_TOKEN`, а в режиме 3 - одноразовую bootstrap-ссылку. Файл `state/control-api.token` тоже не отдавай - коллектор его не собирает (он вне `runs/<id>/`), и это правильно.

## Доступ по сети

Для запуска на фикстурах и на реальных сайтах адрес хоста не нужен: всё выполняется локально. В режиме single-service (рекомендуемом выше) UI и API живут на одном порту `127.0.0.1:8090` и открываются с того же хоста по `localhost`; порт 8088 существует только в split-режиме, где статику отдаёт отдельный сервис `webui`. Адрес хоста в локальной сети (узнать командой `ipconfig`) нужен, чтобы обратиться к сервисам с другого компьютера:

- Открыть UI и API с другого хоста: control-api по умолчанию слушает только `127.0.0.1:8090`. Чтобы отдать его в сеть, задай `CONTROL_API_ADDR=0.0.0.0:8090`, опубликуй порт и добавь правило брандмауэра Windows - тогда `http://<адрес-хоста>:8090` отдаёт и страницы, и API **без каких-либо CORS-настроек**: они на одном origin. Bootstrap-ссылку из лога открывай по тому же адресу хоста (в логе печатается `127.0.0.1`, замени вручную). ⚠ Публичный bind выставляет наружу спавн прогонов; единственный гейт - bearer-токен (ADR-032). Только в доверенной домашней сети.
...
> Это **рабочий лист исполнителя**, а не реестр статуса: отметки ставит тот, кто прогоняет набор у себя.
> Статус самой вехи держит одна запись — `[M9-LIVE]` в `BACKLOG.md` (на 2026-08-21 веха ОТКРЫТА), разбор по
> пунктам с подтверждениями — `docs/M9_LIVE_PLAN.md` §D. Пустой `[ ]` здесь НЕ означает «в проекте не сделано».
...
- [ ] golden байтово стабилен дважды (GAP-RISK-009).
- [ ] лимит бюджета срабатывает с корректной деградацией.
- [ ] собраны реальные значения GAP-RISK-002 (confidence) и GAP-RISK-003 (стоимость и задержка).
