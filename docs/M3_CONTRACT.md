...
| 4 | **инструмент сломался** (ADR-087): наше исключение, вакуумный обход или ненаписанный собственный артефакт — вина `tool`, не `app` |
| 5 | инструмент сломался, но найденное сохранено (ADR-131): обход до места обрыва записан в артефакт |
| -1 | не запустился либо убит сигналом. ⚠ синтетический код control-api, настоящим процессом не возвращается |

> Перечень выводится из `brain/events.json` → `exit_codes` (ADR-141), гейт —
