...
`build` (Go + TS `npm ci`/build + зависимости `uv` + `playwright install`) → матрица `replay` (SQLite для каждого задания);
задание `explore` ручное/`workflow_dispatch`. Проверяет exit codes. Задокументировано, не тестируется локально в runtime.
