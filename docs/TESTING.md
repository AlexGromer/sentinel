...
| **4** | Сломался САМ ИНСТРУМЕНТ: наше исключение, либо обход не увидел вообще ничего (не потому, что смотреть было нечего), либо свой артефакт не записан. Это НЕ находка о приложении |
| **5** | Сломался сам инструмент, но найденное СОХРАНЕНО: план, карта и предложенный сценарий лежат в артефакте, и в них сказано, на каком шаге всё оборвалось |
| **-1** | Процесс не удалось запустить или он был убит сигналом. ⚠ Синтетический код: настоящий процесс им не завершается — его ставит control-api, соединяя `state` с отсутствием кода |

В CI: exit 0 или 1 — нормальные исходы (1 = «тест нашёл проблему»); exit 2 = UI-регрессия; exit 3 = конфигурационная ошибка, требует ручного вмешательства; exit 4 и 5 — **наша** поломка, а не находка о вашем приложении (в шаблонах они дают `unstable`, а не падение).

> Перечень выводится из `brain/events.json` → `exit_codes` и сверяется гейтом
> `tests/test_exit_code_surfaces_offline.py` (ADR-141). Строка про код, которого в каталоге нет, и
> отсутствие строки про объявленный код — оба случая красные.
