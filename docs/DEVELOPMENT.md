...
5. **Перечень ВЫВОДИТСЯ, а не поддерживается.** Проверка не имеет права держать свой список того, что надо покрыть — имена вью, состав сервисов, набор модулей. Причина не в трудозатратах, а в том, КАК такой список отказывает: лишнее в нём видно (запись про удалённое роняет прогон), а **пропущенное — нет**, потому что отсутствие не имеет представления, на которое можно посмотреть. Замерено: смоук интерфейса снимал скриншоты по списку вью, записанному в нём самом; при добавлении новой вью её вписали и не заметили, что двух других там нет и не было никогда. Поэтому: вью берутся из `VIEWS` самого хаба, сервисы для проб и логирования — из состава compose-файлов, маршруты для аудита — обходом таблицы `routes()`. **Обязательный спутник — ПОЛ на число** («вью не меньше девяти», «маршрутов не меньше тридцати»): обход, переставший что-либо находить, пройдёт идеально над пустым множеством, и это единственное, чего сам вывод не ловит. ⚠ Обход дерева задаёт исключения ЯВНО: один `uv sync` не из того каталога превратил обход 37 файлов в обход 4733 и заставил проверку предъявлять требования чужим пакетам.
7. **PR, добавляющий компонент или функциональность, ОБЯЗАН расширить наблюдение за ним** (директива Alex 2026-08-07, в силе с закрытия HEALTH-006). Конкретно и без исключений: проба здоровья и логирование для нового компонента · строка в реестре компонентов, из которого выводится список проб · запись в `docs/capabilities.json` со всеми путями доступа · покрытие в [матрице проверок](PR_ACCEPTANCE.md) (офлайн-сьют, DOM-гейты, скриншоты, живой прогон, докер во ВСЕХ видах поставки, мутации). Причина в том, что наблюдение, добавляемое «потом», не добавляется: компонент попадает в стек, работает, и его отсутствие в пробах обнаруживается в тот день, когда он падает. Компонент без пробы — это компонент, о состоянии которого развёртывание сообщает «здоров» по умолчанию. Отсутствие любого из перечисленных требует ЗАПИСАННОЙ причины рядом с кодом, а не молчания: гейты выводят свои списки из состава развёртывания и таблицы маршрутов, поэтому новый компонент попадает в них по построению и молча пропущен быть не может.
6. **Каждая возможность достижима ТРЕМЯ способами** — из интерфейса, из терминала и по HTTP — либо отсутствие пути записано с причиной. Реестр `docs/capabilities.json` обязан хранить **все** пути записи, а не один: пока их один, реестр отвечает «хотя бы где-то есть», а читается как «покрыто», и принцип нечем измерить.
...
| `LLM_STRUCTURED` | да | `LLM_STRUCTURED_PLANNER` / `_HEAL` | off (opt-in `1` — только OpenAI-совместимый backend; Anthropic всегда native `tool_use`; ADR-057) |
...
- **M9.2b** (`M9.2b_CONTRACT.md`): двухфазный goal (§L) + describe-first (§B) — полный explore→карта сайта→one-shot сценарий, привязанный к реальным элементам (bogus/неоднозначное→unmatched, кросс-страничный navigate синтезируется в коде); `scenario.json` replay'ится детерминированно; богатый RunConfig (auth/scenarios/`--scenario`); offline `test_m9_2b` (20) зелёный + `go build`/`vet`; живой goal/describe-прогон — по «go».

```bash
# offline-набор (без сети/бинарей): весь офлайн-сьют, ОБНАРУЖИВАЕТСЯ глобом.
# Число здесь не пишется — оно протухает быстрее, чем правится: `ls tests/test_*_offline.py | wc -l`
...
1. **agentctl фильтрует env по умолчанию** (`filteredEnv()`, default-on): из хоста в brain проходят только curated-имена (PATH/…, ANTHROPIC_API_KEY/OPENAI_API_KEY/CHECKPOINT_DSN/STORAGE_STATE/…, префиксы `LLM_`/`OTEL_`/`PW_`/`PLAYWRIGHT_`/`SENTINEL_` — **НЕ** `NODE_`/`GIT_` (#25: сужены до точных `NODE_OPTIONS`/`NODE_EXTRA_CA_CERTS`/`GIT_SSL_CAINFO`/`GIT_SSL_CAPATH`, чтобы не пропускать `NODE_AUTH_TOKEN`/`GIT_ASKPASS`), + M11.3 PROM_PUSHGATEWAY/HEAL_VISUAL/SSL_CERT_*/HTTP(S)\_PROXY) **+** имена из `SENTINEL_ENV_ALLOW`. Escape hatch — `SENTINEL_ENV_ALLOWLIST=0` (полный passthrough, для отладки). Функциональные run-vars фильтр не трогает (инъекция после фильтра в `spawnBrain`).
