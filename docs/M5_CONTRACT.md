...
- **PoC gate** (harness heal-poc, вызов вида «agentctl heal-poc --scenarios <dir>»): запускает Tier-7 на ≥20 размеченных сценариях со сломанными
...
> ⚠ **AS-BUILT (2026-09-12, W17).** Три имени этого раздела продукту не соответствовали, и ни одно
> ничем не сверялось — контракт заморожен, а продукт менялся под ним.
> · `--heal-visual` — флага НЕ БЫЛО; соседний `--heal-llm` был реален, поэтому пара читалась как две
>   одинаково существующие ручки, и человек получал код 2, решая, что сломана сборка, а не документ.
>   **Заведён в W17** (решение Alex: принцип 6 закрывается путём, а не отговоркой), так что теперь
>   фраза верна как написана; настройка развёртывания `HEAL_VISUAL` продолжает действовать, когда
>   флаг не передан.
> · вызов «agentctl heal-poc --scenarios <dir>» — подкоманды НЕТ ни среди локальных, ни среди удалённых
>   вербов. Harness не построен; требование ≥70 % остаётся user-run (ARCHITECTURE.md «gated/user-run»).
>   Обратные апострофы с вызова сняты НАМЕРЕННО: код-спан обещает то, что можно набрать.
> · `completeness_ratio < 0.30` — во всём коде ноль вхождений; тир включается четырьмя условиями в
>   `brain/healing.py`, а не полнотой. ARCHITECTURE.md это у себя уже вычистил, носитель обещания —
>   нет.
>
> Имена флагов и подкоманд теперь сверяются гейтом `TestMilestoneContractsNameOnlyThingsTheProgramAccepts`
> (`cmd/agentctl`): он обходит все контракты вех и спрашивает у НАСТОЯЩЕГО флагсета, что программа
> принимает. Следующее такое расхождение краснеет само.
