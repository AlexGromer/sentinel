...
> **New surfaces (M9.6/M9.8/M9-LIVE-prep), not shown in the diagram above (optional/future):** ❽ **CDP-attach** to the user's browser (M9.6, opt-in `PW_CDP_ENDPOINT`), ❾ **browser extension** (M9.8, planned), and ❿ **live-run artifact export** (`scripts/collect-live-run.sh`, M9-LIVE-prep) — see §4.8 / §4.9 / §4.10.
>
...
### 4.9 Boundary ❾ — browser extension (M9.8, PARTIALLY IMPLEMENTED)

> **The MV3 extension itself is not yet implemented** (handed to @0xCoDSnet, #47) — but the transport (WS `/v1/stream`, ADR-043) and **brain-side takeover/return** (R3, ADR-054) ARE implemented, so the transport and takeover-mutation rows below are "done", not "planned". The project's largest surface expansion: an MV3 extension living in the user's browser.
