...
### 4.17 Boundary ⓱ — the screen as a service: the RFB port and the password on disk (LIVE-VNC, ADR-127)

> This surface arrives with the `vnc` profile and DOES NOT EXIST while that profile is off:
> `browser-vnc` does not start, `state/vnc.password` is never created, `CONTROL_API_VNC_LIVE` is empty
> and the `/readyz` probe answers `skipped`. The difference from boundary ⓰ is not one of degree, and
> it is worth saying plainly: there, FRAMES went out behind control-API credentials; here a DESKTOP
> goes out, and it **accepts input**. The second is not a louder version of the first — it is a
> different STRIDE letter.

| Threat (STRIDE) | Vector | Current control | Residual risk |
|---|---|---|---|
| **S/E** — someone else's mouse in a live session | Whoever reaches the RFB port and knows the password gets keyboard and mouse in a browser already logged into the application under test, under the user's cookies. This is not "watching": it is takeover, live from day one by Alex's decision. Everything reachable by the human behind that browser is reachable from there, and the AUT's own audit log will record the user, not a robot | A password is MANDATORY: `-nopw` appears in no shipped file, and a gate asserts that (`tests/test_vnc_profile_offline.py`) rather than a comment promising it. `scripts/vnc-entrypoint.sh` runs under `set -e` and starts x11vnc only after `agentctl vnc-password` returned 0; the verb returns **2** when neither path yields a usable password. The port is NOT published: `browser-vnc` carries no `ports:` key, and the check DERIVES the set of services the rule applies to instead of naming them | Classic VNC authentication checks the **first eight bytes** of the password and does not encrypt the channel: screen contents and keystrokes travel in cleartext. Measured 2026-08-17 against x11vnc 0.9.16: a server holding `ABCDEFGH12345678` accepts `ABCDEFGH` and refuses `ABCDEFGX`. Inside the docker network the boundary is exactly the CDP one (§4.16): "who can reach THIS machine's docker network", not "who can reach localhost". Do not put this stack on a shared/multi-tenant host as-is |
| **I** — the password on disk | `state/vnc.password` is a long-lived secret **in cleartext**, in a directory bind-mounted into every container (`./state:/app/state`) and living on the host | The write is atomic (temp+rename) and `0600` from creation (`os.CreateTemp`); the directory is `0700`; `state/` is in `.gitignore`. An existing but unusable or unreadable file is NEVER overwritten — the process falls back to a throwaway in-memory password and says so, naming the path. Same rule, same reasons, as `state/control-api.token` (§4.11) | **Cleartext is deliberate.** The obfuscated `-rfbauth` format is not encryption (fixed-key DES, reversible by publicly known means): it would buy no confidentiality, while destroying the operator's ability to read their own password and breaking the never-clobber rule — a binary blob is ALWAYS unusable, so no password would survive a single restart. No at-rest encryption (the same level as `state/control-api.token` and `state/golden.key`). On Windows `0600` maps onto ACL semantics |
| **I** — the working copy of the password | x11vnc reads the password from a file, and that file is a SECOND copy of the secret | The copy is written under `/tmp` inside the container, NOT into `./state`: it never reaches the host and dies with the container. Asserted by a gate (`test_the_password_working_copy_never_lands_on_the_host_mount`) | Anyone who can execute a command inside the container reads both it and `state/vnc.password` — but that access is already equivalent to access to the browser |
| **E** — the variable spreading across processes | `SENTINEL_VNC_PASSWORD` exported in the operator's shell would fall under the `SENTINEL_` allow-PREFIX in `cmd/agentctl/main.go::filteredEnv` and reach the brain, the executor and Chromium — i.e. `/proc/<pid>/environ` | A deny set in `filteredEnv`, checked FIRST — before `exact` and before `SENTINEL_ENV_ALLOW`. The same correction shape as #25 after `NODE_AUTH_TOKEN` leaked. In compose the variable is declared ONLY in `browser-vnc`'s own block | `SENTINEL_ENV_ALLOWLIST=0` (full passthrough) bypasses the deny set: that is the documented escape hatch of §4.1, not a forgotten path |
| **T** — the X server as a second door | Xvfb could listen on TCP, and then a second path would lead to the same display — one without even the eight-byte password | `-nolisten tcp`: the X server has no network socket at all, and x11vnc reaches it over `/tmp/.X11-unix/X99` inside the same container. Asserted by a gate | — |
