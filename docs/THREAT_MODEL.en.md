...
|---|---|---|---|---|---|---|
| **Secrets and personal data belonging to the tested application land in our logs.** Seven `app.*` codes (`pw-executor/src/server.ts:79-96`) carry into `runs/<id>/logs/run.jsonl` whatever the application printed itself: a `console.log` with a session token, a URL with a `?token=` query parameter, dialog text containing personal data, a 4xx/5xx response body inside an error message. We do not choose this — what the application prints is not Sentinel's decision. The file is then readable over `GET /v1/runs/{id}/logs` and enters the `collect-live-run.sh` export. | AUT → Chromium → pw-executor → logsink → disk / HTTP / export | **I** | Likelihood: **H** (badly behaved applications print secrets to the console routinely) / Impact: M | **Partial.** The catalogue is fixed — arbitrary strings are not emitted, only seven known shapes. The application channel is **capped at 500 records** (`app.log_capped` announces the truncation, so a truncated capture cannot be mistaken for a complete one). `runs/` and `runs/<id>/` are `0700` (owner-only, #26). HTTP reads are **token-gated** (ADR-032). The `collect-live-run.sh` export **redacts by default** (§4.10). | **There is no write-side redaction.** `logsink.go` stores the message as it arrived; there is no input filter on secret shapes (`Bearer `, `eyJ`, `?token=`, long hex/base64) — unlike `brain`, which redacts its own logs. So `run.jsonl` may hold an application secret in the clear, protected only by `0700` plus the read token. For a regulated buyer that is **not sufficient**: no retention policy is defined and no TTL exists for `logs/`, unlike `trace.zip` which has `SENTINEL_TRACE_KEEP`/`SENTINEL_TRACE_TTL_HOURS`. | **new GAP-SEC-005** / M10 |
