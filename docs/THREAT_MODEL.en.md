...
| **MAIN-world route journal (ADR-138).** The journal lives in the PAGE's world and posts `{__sentinel, url, how, main}` through `window.postMessage`; a page can forge that envelope. Also, a recorded scenario's `expected` now carries the query part of a route. | page → extension → artifact | **S/I** | Lik: L / Impact: M | Two mandatory guards on receipt: `event.source === window` (measured true for MAIN→ISOLATED in Chromium) and the exact envelope name. **The class is NOT new:** a page can already forge a `click` via `dispatchEvent`, which the capture-phase listener records — forging a recorded event was always possible. **Address redaction is mandatory:** the value of a secret-named query parameter is replaced with `REDACTED` in the single `emit()` funnel, for EVERY line — otherwise a token reaches `scenario.json`, a file people commit into their own application's repository. ⚠ Measured on the way: BEFORE ADR-138 a secret in the address reached `runs/record-<session>/events.ndjson` verbatim. | Residual: two routes differing only by a secret parameter become indistinguishable — a deliberate trade against writing the secret down. | **ADR-138 DONE** |
