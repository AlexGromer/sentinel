...
> **Статус:** записан в канон — строка **ADR-056** есть в `ARCHITECTURE.md` §3 и в `ARCHITECTURE.en.md`, статус в таблице ADR — **Proposed** · **Дата:** 2026-07-04 · **Автор:** @AlexGromer + co-architect.
> Этот файл — развёрнутое обоснование границы §2; сама граница РАБОЧАЯ: словарь тиров `🟢OSS`/`🟡SPLIT`/`🔒COMM`/`⚪N/A` размечает открытые пункты `BACKLOG.md` (шапка `## Active` ссылается на ADR-056 §2) и опирается на него `GAPS.md`.
> Одноязычный по решению: файл внесён в `SINGLE_LANGUAGE` (`scripts/check_bilingual.py`), `.en`-зеркала не требуется.
> ⚠ **§2 ревизован ADR-109:** локальная идентичность и мульти-пользователь перенесены из commercial-reserve в OSS-ядро и **реализованы** (`cmd/control-api/access.go`, домен пользователей в `proto/store.proto`); в коммерции остаются OIDC/SSO/RBAC. Таблица §2 ниже этой ревизии не отражает — читать её вместе с ADR-109.
> **Механизмы монетизации не реализованы:** ни верификатор лицензий, ни реестр модулей, ни сервер выдачи лицензий, ни managed-хостинг — это карта решений. Всё, что смержено в основной репозиторий, — Apache-2.0 (`LICENSE`).
