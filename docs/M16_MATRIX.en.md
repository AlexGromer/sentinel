...
of `docs/index.html`, and a live `GET /v1/config-schema`.

## Verdicts

| verdict | meaning | count |
|---|---|---|
| `ui-missing` | not reachable from the UI at all | 22 |
| `ui-only` | exists only in the UI — under the principle in §2.1 this is a gap too (absent from CLI and/or HTTP) | 15 |
| `parity` | present on every surface, equal power | 5 |
| `ui-partial` | reachable from the UI, but expresses less than CLI/HTTP do | 5 |
| `dead-in-ui` | a control exists; its handler does not work | 1 |
| `first-launch-exempt` | bootstrap, deliberately outside the principle | 1 |
...
| capability | verdict | CLI | HTTP | UI |
|---|---|---|---|---|
...
| ↳ **not expressible in the UI:** Arbitrary/external --plan file path. UI/HTTP can only replay a run the server already tracks by run_id (from_run); there is no way to upload or reference a plan.json that did not originate from a run  | | | | |
...
| ↳ **not expressible in the UI:** --target override (baseline against a URL different from the plan's own target_url) and an arbitrary external --plan path; HTTP/UI only accept from_run of a server-tracked run. | | | | |
...
| ↳ **not expressible in the UI:** No per-run override for heal_llm/heal_visual in the Run view or in the POST /v1/runs body — only a persisted global default reachable via the Setup Wizard's PUT /v1/config, which affects every subsequ | | | | |
...
| ↳ **not expressible in the UI:** No stop/cancel control exists anywhere for a run started from the Run/Build view (grepped 'b-stop','bStopRun','b-cancel','bCancel' across docs/index.html — zero hits). Only chat-mode runs are cancella | | | | |
...
| ↳ **not expressible in the UI:** Any file in the run directory NOT in the fixed 13-name whitelist (e.g. raw stdout capture or ad-hoc debug files a specific brain build might drop) is invisible to HTTP/UI and reachable only via direct | | | | |
...
## Run against refutation

Both dangerous claim classes were checked by separate passes, each starting from the position that the
claim is wrong until it fails to be disproved:

- **"absent from the UI"** (22 rows) — none refuted; a false "absent" would cost a rebuild of something
  that already exists;
- **"present in the UI"** — one correction: the retention policy turned out `ui-partial` rather than
  `parity` (the wizard's offline snapshot carries neither `run_keep` nor `run_ttl_hours`).
