...
- `intent` — намерение шага
- `original_locator` — не сработавший `attempted_locator`
- `current_elements` — текущие интерактивные элементы страницы

(промпт `brain/healing.py:121-128` шлёт ровно эти три поля; таблицы неудавшихся стратегий и `element_description` в нём нет)

**Вывод модели** (структурированный JSON, схема `_SCHEMA_CSS` — только CSS-селектор,
без самостоятельно заявленных `confidence`/`reasoning`/`strategy`):
```json
{"css": "<precise CSS selector for the current element>"}
```
или, если ни один элемент не подходит:
```json
{"none": true}
```

**Применяемая скидка:** модель не сообщает собственную уверенность — используется
FIXED-скидка к базовому prior стратегии `css`: `final_confidence = PRIORS["css"] × 0.90 = 0.65 × 0.90 = 0.585`.

**Практическое следствие:** 0.585 < порога ПОМЕЧЕНО (0.60) → результат LLM-повторного связывания
всегда попадает в `needs_review` (Шаг 8), даже если кандидат проходит проверку в живом DOM на
Шаге 7. LLM-повторное связывание никогда не достигает автоматического принятия и никогда не
помечается — оно либо `needs_review`, либо `failed` (если кандидат не разрешается в один элемент
на Шаге 7, `confidence` обнуляется).
