...
> 🌐 **Русский** (основная версия) · [English](M16_MATRIX.en.md)

> **Это снимок 2026-07-29, а не состояние: колонку CLI читать с этой датой.** Диапазоны строк,
> названные ниже, с тех пор уехали — по ним уже не лежит то, что там инвентаризировали. CLI-половину
> закрыл ADR-107: проекция маршрутов на глаголы живёт в `cmd/agentctl/api.go` (`apiVerbs`; маршруты
> без глагола — в `apiRoutesWithoutCLI`, каждый с записанной причиной), и её полноту сторожит
> `cmd/agentctl/api_projection_test.go`. Актуальный ответ на вопрос «есть ли это в CLI» даёт та
> таблица и `agentctl` без аргументов, а не эта карта; перечень глаголов здесь намеренно НЕ
> дублируется — рукописная копия разошлась бы молча, ровно как разошлись строки ниже.
...
- **«присутствует в UI»** — одна поправка: политика удержания оказалась `ui-partial`, а не
  `parity` (офлайновый снимок схемы мастера не несёт `run_keep` и `run_ttl_hours`).
