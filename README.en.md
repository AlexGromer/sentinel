...
| **M3 — CI-Ready Replay** | ✅ done — trust layer, exit codes from the `brain/events.json` catalogue (ADR-141), golden baselines, flake quarantine |
...
| **M11.x — Distribution/install** | ✅ done — release pipeline + Cosign-keyless + syft SBOM (M11.1, ADR-030) · Helm/Flux + Secret plumbing (M11.3, ADR-035) · air-gapped bundle + offline verifier (M11.4) · installer (`install.sh`/`install.ps1`/Homebrew) + schema-driven wizard + config domain + `/readyz` (M11.5, ADR-059..062) · Pages hub + calculators (M11.6/b). **The first signed release `v0.1.0` is out** (2026-08-02, ADR-110): a Release with assets, a multi-arch GHCR image, cosign-keyless, an SBOM; `Formula/sentinel.rb` was rewritten by the `homebrew` job with real urls + sha256 (#183), and `install.sh` resolves `latest`. **What remains of the tail is M11.4 alone:** the full air-gapped bundle (a real GHCR image + model + signatures) is assembled by the maintainer by hand (`scripts/build-airgap-bundle.sh`) — `release.yml` has no job for it, and acceptance criteria 1/3/4/5 are still open. Details — [`docs/DISTRIBUTION.md`](docs/DISTRIBUTION.md) |
...
| **Product revision 2026-07-26** | 🔶 five of the six named gaps lifted, one still open — [`docs/REGRESSION_MAP.en.md`](docs/REGRESSION_MAP.en.md): what the product detects, by what mechanism, and what it does **not**, across three subjects of regression (tool · test · application) against the ГОСТ Р ИСО/МЭК 25010-2015 axes. **The current list of what is open is §9 of the map**; it is not duplicated here. Lifted since: application faults reach the verdict — the **emitter** tallies them, `pass_with_app_faults` (**ADR-072**), and whose fault it was rides a separate `fault_domain` axis (**ADR-113**) · healing reports UI drift as a first-class verdict outcome, `pass_with_drift` (**ADR-071**; the identity of a re-grounded element is still unverified) · test revisions, diff and rollback (**ADR-106**, `agentctl revisions list\|show\|diff\|rollback` — the core; the promote path with a stable `scenario_id` and the golden history remain) · import of an existing suite (**ADR-105**, `agentctl import --from`/`--from-git`, optional `--verify`; Cypress/Selenium remain) · JUnit XML as a run artifact (**ADR-073**). Still open: **the coverage metric is structurally wrong** (`[PROD-CRAWL]` — measured and re-scoped 2026-07-28, the rework is held behind a run against a real 50+ page SPA). Every open backlog item is tagged with a licence tier (ADR-056 §2) |
...
Full design: [`ARCHITECTURE.md`](ARCHITECTURE.md) (ADR journal — §3) · deep-dives in [`docs/`](docs/) · design provenance in [`docs/DESIGN_RECORD.md`](docs/DESIGN_RECORD.md).

> **Browser modes (M9.6):** own-headless by default; `PW_HEADLESS=0` — headed (visible), `PW_CDP_ENDPOINT` — CDP-attach to the user's existing Chrome (the session is reused; the run opens its own tab — ADR-128). The engine is **Chromium-only by design** (ADR-036); deterministic golden replay is headless-only (see [`docs/DETERMINISM.md`](docs/DETERMINISM.md)).
...
| Doc | About |
|-----|-------|
| [`docs/CAPABILITIES.en.md`](docs/CAPABILITIES.en.md) | **what this tool can do** — the catalogue of working features with how to reach each (OpenAI shim · Helm · CDP attach · takeover · MCP server · login-as-test · air-gapped bundle · …), verified by a gate |
